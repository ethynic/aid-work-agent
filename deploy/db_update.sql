...
-- ============================================================================
-- 2026-07-14 删除 payment_orders 表（计费功能未接入，PaymentService 为 Mock 实现）
-- 同步删除 src/saas/api/subscriptions.py、src/saas/services/payment.py 整套代码
-- 保留 subscriptions 表（订阅状态、租户可用子智能体等核心逻辑仍依赖）
-- ============================================================================
DROP TABLE IF EXISTS payment_orders;
DROP INDEX IF EXISTS idx_payment_orders_tenant;


-- ============================================================================
-- 2026-07-14 删除智能体实例并发控制功能
-- 废弃 InstanceService（PostgreSQL 行锁实现）+ AgentInstanceManager（Redis 状态同步）
-- 保留 agent_instances 表业务字段（实例 CRUD），仅删除并发控制相关列与排队表
-- ============================================================================
...
-- 原 idx_agent_instances_tenant / idx_agent_instances_tenant_type 索引包含已删除的 status 列
-- 重建为不含 status 的版本（DROP IF EXISTS 后 CREATE IF NOT EXISTS）
