...
```api-meta
login_url: https://erp10605.aidingyi.cn/api/v1/erp.delegate/login
...
sso_url: https://erp10605.aidingyi.cn/
sso_ticket_param: client_token
sso_grant_url: https://erp10605.aidingyi.cn/api/v1/erp.delegate/sso_grant
sso_login_url: https://erp10605.aidingyi.cn/api/v1/erp.delegate/sso_login
sso_fallback_url: https://erp10605.aidingyi.cn/
```

> **对接硬性要求（每个租户的 pre-sales-api.md 均须满足）**：
> a) 双 Token 机制——`agent_token` 代表智能体身份（环境变量名固定 `AGENT_TOKEN`），用户身份 token（10605 叫 `client_token`，变量名可改，在上方 api-meta 的 `user_token_name` 声明）代表委托用户身份；
> b) 客户表必须有一个字段承载我方 `external_userid`（10605 是 `t_kehuxinxi.unionid`，其他系统表名/字段名可以不同，在上方 api-meta 的 `external_userid_field` 声明）。
...
AI 智能体前端页面（连接中心「外部系统」入口 / 外部接待客户页「打开 10605 ERP」按钮）打开本系统时使用。流程：我方后端按下表接口获取一次性跳转票据 → 前端 `window.open` 打开返回的 `url`（新开窗口）→ 本系统落地页自动完成票据交换（`sso_exchange`，由本系统前端调用，我方无需实现）并无感登录。
...
| 兜底 | `POST /api/v1/erp.delegate/sso_login` | `Api-Authorize-Token: ${AGENT_TOKEN}` | `{"mobile": "<我方登录用户手机号>"}` | 无委托会话可用；手机号须为本系统 `sys_user` 有效用户（status=1 且至少一个有效职位） |
...
```json
{
  ...
  "Response": {
    "client_token": "aad76849c85328b20a537160765fe35f4b2742b597e95a65c3893ef8da52ef88",
    "url": "https://erp10605.aidingyi.cn/?client_token=aad76849c85328b20a537160765fe35f4b2742b597e95a65c3893ef8da52ef88"
  }
}
```
...
| 字段 | 类型 | 说明 |
|------|------|------|
| client_token | string | 一次性跳转票据（64 位 hex），**与委托登录的 client_token 互不通用**（delegate/login 产的 token 不能用于票据交换，反之亦然） |
...
- 失败统一返回 `Code: -1, Error: "单点登录失败"`（不暴露"用户不存在"等细节）。典型原因：我方用户在本系统无账号。此时我方按通用契约**回退**：`window.open` 打开普通登录页（api-meta 的 `sso_fallback_url`），提示用户手动登录，后端记录失败原因供区分「需开通账号」与「临时故障」。
...
| weixinnicheng | string | 否 | 微信昵称；客户信息写入后台时，将客户的**微信昵称**写入该字段 |
| weixintouxiang | string | 否 | 微信头像；客户信息写入后台时，将客户的**微信头像**写入该字段 |
...
| zhuanrengongshijian | datetime | 否 | 转人工时间；用户说完转人工后，记录转人工的时间，格式 `YYYY-MM-DD HH:MM:SS` |
| rengongkefuxingming | string | 否 | 人工客服姓名；用户说完转人工后，记录转到的人工客服的姓名 |
...
3. **修改时必须传递 `update_time`为当前时间（`YYYY-MM-DD HH:mm:ss`）**：若传的时间早于库中 `update_time`，接口返回 `Code: 2`「单据已发生修改」，导致更新失败。**连续多次修改同一条客户记录时的强制要求**，每次提交必须修改 `update_time`为最新当前时间，否则会因为携带旧 `update_time` 触发乐观锁冲突，导致更新失败。
4. **只传需要修改的字段**：避免传递未变更的字段，防止覆盖意外数据
5. **转人工时间和人工客服姓名**：用户可能会多次转人工，每次记录最新的
...
| 字段 | 中文名 | 类型 | 必填 | 控件 | 说明 |
|------|--------|------|------|------|------|
| kehuxingming | 客户姓名 | varchar(100) | 否 | input（关联选择） | 关联【客户信息】模块客户；列表中为客户名称字符串，详情中为 `{label, value}` 对象；**业务上必须传**（不传则记录无法归属客户） |
| kehujiatingdizhi | 客户家庭地址 | varchar(50) | 否 | input | |
| lianxiren | 联系人 | varchar(50) | 否 | 只读 | 由关联客户自动回填 |
| lianxifangshi | 联系电话 | varchar(100) | 否 | 只读 | 由关联客户自动回填 |
| weixinhao | 微信号 | varchar(50) | 否 | 只读 | 由关联客户自动回填 |
| suoshuhangye | 所属行业 | varchar(50) | 否 | 只读 | 由关联客户自动回填 |
| shijian | 本次跟进时间 | date | 否 | date | 业务上必传，`YYYY-MM-DD` |
| genjinren | 跟进人 | varchar(100) | 否 | input | 业务上必传，填智能体或委托人名称 |
| xiaciriqi | 下次跟进时间 | date | 否 | date | 聊天中约定下次跟进时填 |
| neirong | 推广内容摘要 | varchar(100) | 否 | input | 智能体发送内容的摘要 |
| kehuhuifuneirong | 客户回复内容 | text | 否 | textarea | 客户回复的摘要 |
| genjindongzuo | 跟进动作 | longtext(200) | 否 | textarea | 如"首次触达/报价/邀约" |
| beizhu | 备注 | text(100) | 否 | textarea | |

系统字段：`id`、`sid`、`create_user`、`create_group`、`update_user`、`create_time`、`update_time`、`status`（隐藏）、`status_approve`（隐藏）。
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| kehuxingming | string | 否 | **客户记录 id**（不是客户名称文本），取自客户列表 `data[].id` 或创建客户接口返回值 |
...
| xiaciriqi | string | 否 | 下次跟进时间；**仅当客户在对话中明确约定了下次聊天/跟进时间**时填写，格式 `YYYY-MM-DD`，未明确约定则留空 |
...
> **`kehuxingming` 字段约束**：该字段为关联字段，后端校验其值必须是【客户信息】模块中已存在客户的记录 id。传客户名称文本或不存在 id 会写入失败。**客户基本信息字段**：`lianxiren`、`lianxifangshi`、`weixinhao`、`suoshuhangye` 虽然在数据字典中标记为"只读"（表示前端编辑表单中不可手动修改），但创建跟进记录时，AI 智能体**必须主动从客户信息中查询并传入**这些字段。后端不会自动同步这些值。
...
| 21 | status_approve | 审批状态 | int | 50 | | | 隐藏 | input |
| 22 | unionid | 外部用户唯一标识 | varchar | 50 | | | | input |
...
**更新时机**：客户提供了新的手机号、微信号或可确认的身份信息时，把变化字段通过修改接口更新（只传变化字段 + `id` + `update_time`）。**同一会话中多处客户信息修改，尽可能合并提交一次修改接口**，例如客户提供了新手机号、微信昵称也有变化、跟进汇总摘要也发生变化，尽可能合并提交一次客户修改接口。
