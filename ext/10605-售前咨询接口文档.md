...
<!-- api-meta：系统代码解析的机器可读约定块，请勿删除本块或改键名（login_url 必填；user_token_name / external_userid_field 缺省时分别默认 client_token / unionid；push_exclude_sections 可选，声明推送循环注入 LLM 时裁剪的章节标题，逗号分隔） -->
```api-meta
