...
委托登录流程：用户提供手机号 -> AI 智能体调 `login` 获取 `client_token` -> 业务接口携带双 token -> 会话结束调 `logout`（`POST /api/v1/erp.delegate/logout`，需 `Api-Authorize-Token` + `Client-Authorize-Token`，返回 `{"msg":"已登出"}`，之后该 `client_token` 立即失效）。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| client_token | string | 委托人 token，后续业务接口放在 `Client-Authorize-Token` Header；服务端为文件缓存、无内置过期时间，仅在 logout、委托人被禁用或缓存目录被运维清理时失效，建议 AI 智能体会话结束主动调 logout |
...
|--------|------|------|------|
| module | string | 是 | 固定为 `"kehuxinxi"` |
| did | null | 是 | 创建时固定为 `null` |
| tables | array | 是 | 包含主表数据 |
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| xingming | string | 是 | 客户名称 |
| lianxiren | string | 是 | 联系人 |
| lianxidianhua | string | 否 | 联系电话 |
| weixinhao | string | 否 | 微信号 |
| suoshuhangye | string | 是 | 所属行业，从选项值中选取 |
| tuiguangriqi | string | 是 | 推广日期，格式 `YYYY-MM-DD` |
| tuiguangqudao | string | 是 | 推广渠道，从选项值中选取 |
...
|--------|------|------|------|
| module | string | 是 | 固定为 `"kehuxinxi"` |
| did | int | 是 | 客户记录 id（修改时必填） |
| tables | array | 是 | 仅包含需要修改的字段 |
| temp | bool | 是 | 是否临时保存，固定为 `false` |
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| module | string | 是 | 固定为 `"lianxijilu"` |
| filters | array | 否 | 过滤条件，参见第 1 节 filters 约定 |
| page, limit | - | - | 参见第 1 节分页参数 |
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| kehuxingming | string | 是 | 客户姓名，从客户信息中获取 |
...
| shijian | string | 是 | 本次跟进时间，格式 `YYYY-MM-DD`，应为当前日期 |
| genjinren | string | 是 | 跟进人，默认取委托人 display_name |
...
| 创建客户信息时 | `xingming`（客户名称）为必填，`lianxiren`（联系人）为必填 |
...
|------|--------|--------|----------|----------|------|------|------|------|
| 1 | xingming | 客户名称 | varchar | 100 | 必填 | | | input |
| 2 | lianxiren | 联系人 | varchar | 50 | 必填 | | | input |
| 3 | lianxidianhua | 联系电话 | varchar | 100 | | | | input |
| 4 | weixinhao | 微信号 | varchar | 50 | | | | input |
| 5 | suoshuhangye | 所属行业 | varchar | 50 | 必填 | | | select |
| 6 | tuiguangriqi | 推广日期 | date | 50 | 必填 | | | date |
| 7 | tuiguangqudao | 推广渠道 | varchar | 50 | 必填 | | | select |
...
| 10 | account | 用户名 | VARCHAR(50) | 100 | 必填 | | | |
| 11 | confidential | 加密密码 | VARCHAR(300) | 100 | 必填 | | | |
| 12 | name | 真实姓名 | VARCHAR(50) | 100 | 必填 | | | |
| 13 | mobile | 手机号 | VARCHAR(11) | 100 | 必填 | | | |
...
| 8 | update_user | 修改人 | int | 100 | | | | |
| 9 | status_approve | 审批状态 | int | 100 | | | 隐藏 | |
| 10 | sys_user_id | 用户 | INTEGER | 100 | 必填 | | | |
| 11 | sys_role_id | 角色 | INTEGER | 100 | 必填 | | | |
| 12 | sys_group_id | 部门 | INTEGER | 100 | 必填 | | | |
| 13 | is_admin | 是否：管理员 | INTEGER | 100 | 必填 | | 隐藏 | select |
| 14 | is_disable | 禁用 | INTEGER | 100 | 必填 | | | select |
| 15 | is_main_post | 是否主职位 | INTEGER | 100 | 必填 | | | select |
