...
预置吞吐（PTU）部署同样支持上下文缓存。命中缓存时，PTU 额度消耗按缓存折扣系数折算。详见[预置吞吐长输入与缓存](#)。
...
千问 Max：qwen3.8-max、qwen3.7-max、qwen3.7-max-2026-05-20、qwen3.7-max-2026-06-08、qwen3.6-max-preview、qwen3-max
...
-   **全球服务部署范围：**
    ...
    千问 Max：qwen3.8-max、qwen3.7-max、qwen3.7-max-2026-05-20、qwen3.7-max-2026-06-08、qwen3-max
    
    千问 Flash：qwen3.7-flash、qwen3.7-flash-2026-07-15、kimi-k2.7-code
...
-   **全球服务部署范围：**
    ...
    千问 Flash：qwen3.7-flash、qwen3.7-flash-2026-07-15、qwen3.5-flash、qwen-flash
...
-   **全球服务部署范围：**
    ...
    千问 Max：qwen3.8-max、qwen3.7-max、qwen3.7-max-2026-05-20
    
    千问 Plus：qwen3.7-plus、qwen3.7-plus-2026-05-26、qwen3.6-plus
    
    千问 Flash：qwen3.7-flash、qwen3.7-flash-2026-07-15、qwen3.6-flash
...
-   **例外**：qwen3.8-max 的显式缓存命中价格不是标准输入单价的 10%，具体价格请参见百炼控制台（缓存创建价格仍为标准单价的 125%）。
...
-   **文本生成模型**
    ...
    -   Kimi（阿里云百炼部署）：kimi-k2.7-code、kimi-k2.6、kimi-k2.5、kimi-k2-thinking、Moonshot-Kimi-K2-Instruct
...
-   **文本生成模型**
    ...
    -   千问 Max：qwen3.8-max、qwen3.7-max、qwen3.7-max-2026-05-20、qwen3.7-max-2026-06-08、qwen3-max、qwen-max
...
-   **全球服务部署范围：**
    ...
    -   文本生成模型
        ...
        -   千问 Flash：qwen3.7-flash、qwen3.7-flash-2026-07-15
...
阿里云百炼部署的模型触发隐式缓存的最少 Token 数为 256，Qwen3.7系列模型约为2000。智谱部署的GLM、稀宇科技部署的 MiniMax 模型为 512。
...
-   阿里云百炼部署的模型（deepseek-v4-pro、qwen3.8-max 除外）：`cached_token` 单价为 `input_token` 单价的 **20%**
...
-   GLM（智谱部署）：ZHIPU/GLM-5.2、ZHIPU/GLM-5.1、ZHIPU/GLM-5均为 **25%**
...
-   **模型监控：**在百炼控制台「[模型监控](https://bailian.console.aliyun.com/cn-beijing?tab=model#/model-telemetry)」页面，选择目标模型和时间范围，查看 `cache_tokens` 指标（支持按分钟/按小时精度统计）。
    
-   **账单明细：**在百炼控制台「账单详情」页面导出账单明细，按计费类型筛选"缓存命中"（`cache`）相关记录，按日期分组汇总，可获取每日的缓存命中 Token 计量数据。详细操作请参见[账单查询与成本管理](https://help.aliyun.com/zh/model-studio/bill-query-and-cost-management)。