...
def generate_report(requirement: str, result: dict, report_dir: str):
    ...
    report_path = os.path.join(report_dir, f"scenario_17_report_{ts}.md")

    lines = []
    lines.append("# 场景17 测试报告\n\n")
...
async def main():
    ...
    requirement = "帮我看下每个合同的累计回款金额和发票金额"

    analyzer = DataAnalyzer(session_id=f"test_s17_{int(time.time())}")
    analysis_id = f"test_s17_{int(time.time())}"

    print("=" * 60)
    print(f"场景17: {requirement}")
