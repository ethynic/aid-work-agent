...
@pytest.mark.asyncio
async def test_hallucinated_value_or_quote_is_inconclusive():
    data = empty_profile()
    data["address"] = {
        "value": "不存在的地址",
...
@pytest.mark.asyncio
async def test_mobile_must_be_bound_to_same_name_in_quote():
    content = "秘书长王承展\n联系人李雷 18511597486"
    data = empty_profile()
    data["secretary_general_name"] = {
...
@pytest.mark.asyncio
async def test_president_mobile_binding_remains_strict_after_field_retirement():
    content = "会长许萍\n秘书长王承展 18511597486"
    data = empty_profile()
    data["president_name"] = {
...
@pytest.mark.asyncio
async def test_mobile_cannot_be_cross_bound_inside_multi_contact_quote():
...
@pytest.mark.asyncio
async def test_source_url_query_must_match_exact_verified_page():
...
@pytest.mark.asyncio
async def test_quote_cannot_be_assembled_across_pages():
    first = page("秘书长 王承展")
...
@pytest.mark.asyncio
async def test_empty_strings_normalize_to_null_but_partial_null_evidence_fails():
    clean = empty_profile()
    ...
    assert partial_result.status == "inconclusive"


@pytest.mark.asyncio
async def test_missing_profile_field_is_rejected():
    data = empty_profile()
...
@pytest.mark.parametrize(
    ...
    [
        ("12", "协会设有12个分支机构", "success"),
        ("12家", "协会有12家单位会员", "inconclusive"),
        ("十二", "协会连续举办十二届品牌会议", "inconclusive"),
        ("1", "协会成立于2021年", "inconclusive"),
    ],
)
async def test_count_normalization_is_conservative(value, quote, expected):
    data = empty_profile()
    data["branch_count"] = {
...
@pytest.mark.parametrize(
    ("value", "quote", "expected"),
    [
        (16, "协会共有16名工作人员", "success"),
        (16, "协会设有15个分支机构", "inconclusive"),
        (1, "协会设有100个分支机构", "inconclusive"),
    ],
)
async def test_integer_normalization_keeps_exact_numeric_token_check(
    value, quote, expected,
):
    data = empty_profile()
    data["branch_count"] = {
...
@pytest.mark.parametrize(
    ("field_name", "value", "quote"),
    [
        ("branch_count", True, "协会设有1个分支机构"),
        ("branch_count", False, "协会设有0个分支机构"),
        ("branch_count", 1.5, "协会设有1.5个分支机构"),
        ("branch_count", -1, "协会设有-1个分支机构"),
        ("president_name", 1, "会长1"),
    ],
)
async def test_numeric_values_outside_integer_count_contract_fail_closed(
    field_name, value, quote,
):
    data = empty_profile()
    data[field_name] = {
...
@pytest.mark.asyncio
async def test_scientific_notation_count_fails_closed():
    data = empty_profile()
    data["branch_count"] = {
        "value": 1000.0,
...
@pytest.mark.asyncio
async def test_email_requires_email_context_and_valid_format():
    data = empty_profile()
    data["email"] = {
        "value": "not-an-email",
