...
验证 _can_download_document：本租户放行、demo 模式回归、共享范围校验（§6.5 下载漏洞修复）。
"""
...
def test_can_download_demo_without_tenant_context():
    """无租户上下文（demo/命令行）：demo 或无租户文档放行"""
    assert _can_download_document({"tenant_id": "demo", "source_type": "product"}, None) is True
    assert _can_download_document({"tenant_id": None, "source_type": "product"}, None) is True


def test_cannot_download_demo_with_tenant_context():
    """有租户上下文 B，但文档归属 demo 且无共享授权 -> 拒绝"""
    with patch("src.knowledge.api._has_shared_access", return_value=False):
        assert _can_download_document({"tenant_id": "demo", "source_type": "product"}, "B") is False
