...
async def test_whitespace_only_content_treated_as_failure(service, mock_llm_for_summary, monkeypatch):
    ...
    mock_llm_for_summary.chat = AsyncMock(return_value={"content": "   \n\t  \n", "tool_calls": None})
...
async def test_long_content_preserved(service, mock_llm_for_summary, monkeypatch):
    ...
    mock_llm_for_summary.chat = AsyncMock(return_value={"content": long_summary, "tool_calls": None})
    result = await service._call_summary_llm(None, [{"role": "user", "content": "x"}])
...
async def test_existing_summary_none_vs_nonempty_both_pass(
    ...
    # 1) None
    mock_llm_for_summary.chat = AsyncMock(return_value={"content": "fresh summary", "tool_calls": None})
    result_none = await service._call_summary_llm(None, [{"role": "user", "content": "x"}])
    assert result_none == "fresh summary"

    # 2) 非空
    mock_llm_for_summary.chat = AsyncMock(return_value={"content": "merged summary", "tool_calls": None})
    ...
    second_call_kwargs = mock_llm_for_summary.chat.call_args.kwargs
...
async def test_llm_returns_none_result_treated_as_failure(
    ...
    monkeypatch.setattr(asyncio, "sleep", AsyncMock(return_value=None))
    mock_llm_for_summary.chat = AsyncMock(return_value=None)
    result = await service._call_summary_llm(None, [{"role": "user", "content": "x"}])
    assert result is None
    assert mock_llm_for_summary.chat.await_count == 3
...
async def test_llm_result_missing_content_key_treated_as_failure(
    ...
    monkeypatch.setattr(asyncio, "sleep", AsyncMock(return_value=None))
    mock_llm_for_summary.chat = AsyncMock(return_value={"tool_calls": None})  # 无 content
    result = await service._call_summary_llm(None, [{"role": "user", "content": "x"}])
    assert result is None
