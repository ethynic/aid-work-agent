"""compress_session 真实路径 mutation 加强测试（v3.1 Phase 3）
...
- compress_session 真的把 _resolve_session_meta 的 context_token_count 传给 _should_compress
- force=True 时 _should_compress 不被调用（不是「调用了但被忽略」）

如果某天有人把 compress_session 的 force 逻辑改成「调 _should_compress 但忽略结果」，
现有测试不会失败，但本文件会。
"""
...
async def test_force_skips_should_compress_by_not_calling(service, monkeypatch):
    """force=True 时 _should_compress 应完全不被调用。

    使用「让它抛异常」的方法验证，比现有「patch return value」更严格。
    mutation: 如果有人改成 `if force and not should_compress_result` 这种语义，
    此测试会失败。
    """
...
@pytest.mark.asyncio
async def test_context_token_count_actually_passed_to_should_compress(
    service, monkeypatch
):
    """_resolve_session_meta 返回的 context_token_count 真的传给了 _should_compress。
    ...
    async def _fake_load(sid, st, tid):
        return []  # 不需要消息，_should_compress 直接被 cache 触发

    service._resolve_session_meta = _fake_meta
    service._load_messages = _fake_load
    service._model_limit_cache = 100_000  # 模型上限 100K，cache=99999 < 70K 阈值不触发

    # 替换 _should_compress，记录参数
    def _spy_should(messages, cached_tok, model_limit):
        captured["cached_tok"] = cached_tok
        return False, "spy"

    service._should_compress = _spy_should

    result = await service.compress_session("sess", "chat")
    assert result is None
    # 验证：_should_compress 拿到的 cached_tok 来自 meta.context_token_count
    assert captured["cached_tok"] == 99999, (
        "compress_session 必须把 meta.context_token_count 传给 _should_compress"
    )


@pytest.mark.asyncio
async def test_get_model_limit_actually_called_when_force_false(service, monkeypatch):
    """force=False 时 _get_model_limit 被调用（验证 compress_session 真的解析 model_limit）。
    ...
    async def _fake_load(sid, st, tid):
        return [{"role": "user", "content": "hi", "id": 1}]
    ...
    assert called["n"] >= 1, "_get_model_limit 必须在 force=False 时被调用"


@pytest.mark.asyncio
async def test_force_true_does_not_call_get_model_limit(service):
    """force=True 时不调 _get_model_limit（验证优化：跳过整个阈值检查分支）"""
    async def _fake_meta(sid, st):
        from src.memory.mid_term import SessionMeta
