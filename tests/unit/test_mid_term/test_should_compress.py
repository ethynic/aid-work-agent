...
def test_below_both_thresholds_returns_false(service, build_simple_messages):
    """未达任何阈值 → False"""
    # model_limit = 128K，10 条短消息远不到阈值
    msgs = build_simple_messages(10)
    should, reason = service._should_compress(msgs, model_limit=128_000)
...
def test_token_threshold_only_returns_true(service):
    ...
    should, reason = service._should_compress(msgs, model_limit=10_000)
...
def test_message_count_threshold_only_returns_true(service):
    ...
    should, reason = service._should_compress(msgs, model_limit=1_000_000)
    assert should is True
