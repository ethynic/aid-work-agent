...
重点覆盖：
- 计费 ×5 公式（record_llm_usage：raw_credit × multiplier = credit_cost）
- 激活码生成格式与 hash 校验
...
class TestClientCreditMultiplier:
    ...
    def test_multiplier_default_is_5(self):
        """默认系数 5.0"""
        assert _client_credit_multiplier() == 5.0
...
class TestRecordLlmUsageBilling:
    """record_llm_usage 计费公式：raw_credit × 5 = credit_cost，2位小数向上取整。"""

    @patch("src.db.client_binding_db.calculate_credit_cost")
    @patch("src.db.client_binding_db.get_db_connection")
    def test_credit_cost_is_5x_raw(self, mock_conn, mock_calc):
        """credit_cost = ceil(raw_credit × 5 × 100) / 100"""
    ...
    def test_credit_cost_rounds_up(self, mock_conn, mock_calc):
        ...
        # ceil(0.001 × 5 × 100) / 100 = ceil(0.5) / 100 = 1/100 = 0.01
    ...
    def test_balance_after_returned_when_deducted(self, mock_conn, mock_calc):
        """扣费后返回 balance_after"""
        mock_calc.return_value = 1.0  # raw=1, cost=5
        cursor = MagicMock()
        cursor.fetchone.return_value = {"credit_balance": 95.0}
        mock_conn.return_value.__enter__.return_value.cursor.return_value = cursor
        ...
        assert result["balance_after"] == 95.0
