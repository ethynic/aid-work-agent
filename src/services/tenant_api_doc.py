"""租户 API 文档（pre-sales-api.md）api-meta 公共读取入口
...
# system_id 固定值：当前外部系统列表唯一来源是售前租户文档，未来多来源时扩展
PRE_SALES_SYSTEM_ID = "pre_sales"


def tenant_doc_path(tenant_id: str) -> Path:
...
def load_tenant_doc(tenant_id: str) -> Optional[str]:
    """读取租户接口文档全文；缺失返回 None"""
    path = tenant_doc_path(tenant_id)
    if not path.exists():
        return None
    try:
        return path.read_text(encoding="utf-8")
    except Exception as e:
        logger.warning(f"[external_systems] 租户文档读取失败 tenant={tenant_id}: {e}")
        return None
...
def parse_sso_config(meta: Dict[str, str]) -> Optional[Dict[str, Any]]:
    """从 api-meta 键值解析外部系统入口（SSO）配置

    返回 None：未声明 sso_enabled/sso_url（该租户不显示入口）。
    返回 dict：sso_ready 表示入口是否可用（不可用时前端按钮禁用并提示）。
    未知 sso_mode 不猜——按 sso_ready=False 处理。
    """
...
def load_sso_config(tenant_id: str) -> Optional[Dict[str, Any]]:
    """读取租户文档并解析 SSO 配置的组合入口（文档缺失返回 None）"""
    doc = load_tenant_doc(tenant_id)
    if not doc:
        return None
    return parse_sso_config(extract_api_meta(doc))
