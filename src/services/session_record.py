    ...
    try:
        ...
        if not llm_model:
            try:
                ...
                llm_model = getattr(_settings.memory.mid_term.summary_llm, "model", None) or "deepseek-flash"
            except Exception:
...
def _persist_skill_llm_record(
    ...
    *,
    ...
    # 产出，provider 可能是 qwen/zhipu，缺失时才兜底 deepseek-flash——单价按模型取，传错即错价）
    llm_model = model or str(usage.get("model") or "") or "deepseek-flash"
