...
# 列表页轻量字段（不含 ocr_text，全文可能很大）
_LIST_COLUMNS = (
    "id, tenant_id, user_id, candidate_name, job_name, candidate_info, images, "
    "source, status, remark, fetched_at, created_at, updated_at"
)
...
def _row_to_resume(row) -> Dict[str, Any]:
    """行记录转 dict，JSONB 字段兜底解析（candidate_info/images）"""
    item = dict(row)
    ...
    *,
    ...
    with get_db_connection() as conn:
        ...
        cursor.execute(
            ...
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, 'new', %s, COALESCE(%s, CURRENT_TIMESTAMP))
            RETURNING *
            """,
            (
                tenant_id, user_id, candidate_name.strip(), job_name,
    ...
    logger.info(f"简历入库: tenant={tenant_id}, candidate={candidate_name}, "
                f"job={job_name}, source={source}, images={len(merged_images)}")
