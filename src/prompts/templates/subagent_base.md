...
**避免重复加载（节省 token 和工具调用）**：
- `use_skill` 返回的操作指南会一直保留在上下文中。只要之前已经加载过某技能的完整指南，后续（无论是否调用过 `skill_complete`）都**不要**再次 `use_skill` 同一个技能，直接按上下文里已有的指南执行即可。
- **唯一需要重新 `use_skill` 的情况**：之前那次加载只读取了技能的**部分内容**（例如技能包含多个子模式/章节，第一次只加载了其中一个），而现在需要用到其他部分——此时按需重新调用 `use_skill` 指定要加载的范围。
