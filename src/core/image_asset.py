...
class ImageRegistry:
    ...
    async def cleanup_temp(self, older_than_hours: int = 24) -> int:
        ...
        ⚠️ 未来启用定期清理时的保留期要求：本方法会物理删除磁盘文件，而
        /api/files/{file_id}/download 在 Redis 元数据过期后依赖磁盘扫描回退来
        持续服务历史消息（如外部接待页）中的图片。因此若将本方法接入定时任务，
        older_than_hours 必须设置 6 个月（约 4380 小时）以上的保留期，否则历史
        消息中引用的工具生成图片将永久失效。当前本方法无任何调度入口（死代码），
        未启用前所有磁盘文件均持久保留。
