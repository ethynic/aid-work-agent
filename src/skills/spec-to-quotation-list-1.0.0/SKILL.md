...
- **单文件体积目标 3~5MB**（询价单经邮件/微信传输，图片是体积大头，约 100~150KB/张）：
  单个拆分文件预估条目 > 50 或图片 > 30 张时，继续按空间部位细分（如"客房"再拆成
  客房-饰面 / 客房-家具 / 客房-卫浴）。**优先用拆分控制体积，而不是靠压缩图片**。
...
**探查必须批量进行，严禁一次调用只探一小段**：一次调用可传任意多个页段（支持 `6-10` 区间写法），
例如 `python scripts/probe_pdf.py <pdf> 6-20 31-45`。全书正文通常 **1~2 次调用**探完，
每次调用固定消耗一轮智能体迭代，逐页/逐小段调用会在大 PDF 上把迭代轮数耗尽导致任务中断。

**不要 `read` 脚本源码**：各脚本的用法、参数、输入输出约定已全部写在本 SKILL.md 和
`references/` 文档中，按上面的命令直接执行即可。读 500+ 行的脚本源码既浪费迭代轮数，
输出截断后也得不到完整信息。
...
体积控制**首选第 1 步的主动拆分**（单文件 3~5MB）；`--size-budget-mb` 仅作兜底——
生成后单文件仍超预算时自动降质重嵌（dpi 与 JPEG 质量各乘 0.8，最多 4 轮，下限 60），
也可用 `--dpi` / `--jpg-quality` 一开始就压低质量。
...
**交付前必须产出任务台账** `$OUT/summary.md`，并与其他交付物一起 cp 注册下载。
台账是跨会话续作的唯一凭据（用户后续"填参数重算/补数据"全靠它），固定包含：

1. 源文件与技能版本（PDF 文件名、页数、文档类型）
2. 交付清单：拆分文件名 + 各文件条目数 / 分组数 / 已算量占比（直接摘抄 validate 输出）
3. **关键参数快照及推导依据**：每个已填参数的取值、来源（手册页码）或推导式
   （如"客房走廊 1285 ㎡ = 客房总数 × 走廊宽 1.5 m 推导"）
4. **待填参数清单**：留空参数的名称、影响哪些文件、需要谁提供
5. 待办事项：需业主/用户确认的事项逐条列出
