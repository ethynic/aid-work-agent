...
### 第2部分：门票价格表
仅包含**进入景区的门票**价格，一行一个价格，**价格字段必须标注"挂牌价"/"结算价"**：
研学体验票 | 学生/成人 | 挂牌价:80元 | 结算价:80元 | 含材料费 | 单次体验
...
7. **第2部分的价格字段必须标注"挂牌价"/"结算价"**（如"挂牌价:80元 | 结算价:80元"而非裸数字"80 | 80"），其他字段无需加列名前缀
