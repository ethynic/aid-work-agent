...
### 第2部分：价格明细表
包含所有房型的价格信息，**每个房型+每个日期区间一行**，**第一行必须是表头**：
房型 | 散客价 | 团客价 | 含早 | 适用日期 | 备注

**必须输出表头行**（值为 `房型 | 散客价 | 团客价 | 含早 | 适用日期 | 备注`），后续每一行是一个房型+日期区间的价格数据。表头行不能省略、不能改名。

**列含义（六列，用 | 分隔）**：

| 列序 | 列名 | 说明 |
|------|------|------|
| 1 | 房型 | 房型完整名称。**必须合并床型/面积/景观等描述信息**（见下方"房型解析规则"） |
| 2 | 散客价 | 散客/门市价/参考价/网站价等**对外非团队价格**（单位：元/间/夜） |
| 3 | 团客价 | 团队价/团散同价/旅行社协议价等**团队对外价格**（单位：元/间/夜） |
| 4 | 含早 | 含双早/含单早/不含早/未知 |
| 5 | 适用日期 | 日期范围（已展开为具体日期区间） |
| 6 | 备注 | 其他需要保留的说明（如"团散+20"、"节假日专用"、"执行16免1"等） |

### 房型解析规则（重要！）

1. **房型名必须合并所有相关信息**：原始数据中房型名常与"床型"、"面积"、"景观"等分列存放，必须合并为完整一行
   - 例如原始数据有：`高级城景房 | 1.8米大床 | 35㎡ | 城景` → 房型列合并为 `高级城景房（1.8米大床·35㎡·城景）`
   - 例如原始数据有：`标准间 | 双床1.2米 | 28平米` → 房型列合并为 `标准间（双床1.2米·28㎡）`
2. **合并时用中圆点 `·` 分隔各属性，用括号 `（）` 包裹属性部分**
3. **没有附加属性时不要加括号**：房型列就是 `标准间`、`大床房` 等纯名称
4. 同一酒店内房型名必须唯一
...
把原始 Excel 中的每个价格列归入下面三类之一：

| 类别 | 包含的列名关键词 | 处理 |
|------|----------------|------|
| **内部价（禁止解析）** | 结算价、成本价、底价、给社价、内部价、采购价、旅行社结算、同行价、拿货价、Net、Cost | **整列丢弃**，任何情况下都不要把内部价填进散客价/团客价 |
| **对外价 - 散客倾向** | 散客价、散客、前台价、挂牌价、网站价、参考价、Rack rate | 填到"散客价"列 |
| **对外价 - 团队倾向** | 团队价、团队、团散同价、旅行社价、协议价、Group | 填到"团客价"列 |
| **对外价 - 通用**（既不明确散客也不明确团队） | 门市价、标准价、报价、房价、现付价、面议价等 | 见下方"门市价等通用对外价的处理" |

**门市价等通用对外价的处理（关键！）**

门市价是**酒店对外挂牌价**，不属于内部价。当 Excel 中**同时**存在门市价和明确的散客价/团客价时，门市价填到散客价列（散客对外价通常 = 门市价或其折扣）；当 Excel 中**只有门市价一种对外价**（无论还伴随什么内部价如结算价），说明该酒店没有区分散客/团队对外报价，此时**门市价同时填到散客价列和团客价列**（两列同值）。

**绝对禁止的做法**

❌ 把"结算价/成本价/底价"等内部价填进散客价或团客价列 —— 即使另一列空着也不行
❌ 看到"门市价 + 结算价"两列时，把门市价填散客价、结算价填团客价 —— 结算价必须丢弃，门市价应同时填两列
❌ 当一种对外价都没有但只有内部价时，仍然输出该房型 —— 应整行丢弃

**正确做法**

✅ 看到"门市价 + 结算价" → 散客价=门市价，团客价=门市价，结算价丢弃
✅ 看到"散客价 + 团队价" → 散客价列填散客价，团客价列填团队价
✅ 看到"散客价 + 团队价 + 结算价" → 散客价列填散客价，团客价列填团队价，结算价丢弃
✅ 看到"门市价 + 团队价" → 散客价列填门市价，团客价列填团队价
✅ 只有"结算价"一列 → 该房型整行丢弃（没有对外价）

**其他规则**

1. 如果"团散+20"等差价规则 → 团客价 = 散客价 + 差额，直接计算结果；备注列注明"团散+20"
2. **完全没有对外报价（只有内部价或无价）的房型整行丢弃**，不要输出
3. 价格只保留数字（含小数），不要单位、货币符号

### 日期与备注规则

1. 所有模糊日期（如"五月"、"7-8月"）转换为具体日期范围（如"5月1日-5月31日"）
2. 节假日单独列出行（五一、国庆、端午等），备注列注明"节假日专用"
3. 司陪房、免房政策等特殊规则不进入房型行；如有，附在价格表末尾，行首为"特殊政策："
4. "门市价8.5折"等无法精确计算的，备注列保留原文
5. 如果某月份列无数据，则跳过该月份
...
[
  {{
    ...
    "price_table_text": "价格明细表文本（一行一个房型+日期区间，六列）",
    "metadata": {{
      "sub_region": "子区域",
      "diamond_level": "4钻",
      "address": "地址",
...
输出必须尽量精简，控制在合理长度内：
1. **相同房型+相同散客价+相同团客价+相同含早的连续月份必须合并为一行**，如"5月1日-10月31日"而非每月单独一行。例如：
   - ✅ 高级城景房（1.8米大床·35㎡） | 580 | 450 | 含双早 | 5月1日-6月30日 |
   - ❌ 高级城景房（1.8米大床·35㎡） | 580 | 450 | 含双早 | 5月1日-5月31日 |
   - ❌ 高级城景房（1.8米大床·35㎡） | 580 | 450 | 含双早 | 6月1日-6月30日 |
2. **散客价和团客价相同时仍各占一列**（不要省略其中一列），下游按列解析，两列都填同一数字
3. 省略值为"未知"或空的备注列可以留空（分隔符 | 仍要保留，保持六列结构）
4. 只保留关键价格信息，去掉重复描述
5. 非连续月份但价格相同的也应尝试合并，如「5月、9月」标为「5月1日-5月31日、9月1日-9月30日」

## 价格明细表示例

正确的六列输出示例（注意：**第一行是表头，必须原样输出**；之后每行必须恰好 6 个 `|` 分隔的字段）：
...
标准间（双床1.2米·28㎡） | 680 | 530 | 含双早 | 10月1日-10月7日 | 节假日专用
特殊政策：16免1，司陪房200元/晚
```

**只有门市价一种对外价时的正确写法**（散客价和团客价两列同值，结算价已被丢弃）：

```
房型 | 散客价 | 团客价 | 含早 | 适用日期 | 备注
标准间（双床1.2米·28㎡） | 480 | 480 | 含双早 | 5月1日-10月31日 |
大床房（1.8米大床·30㎡） | 580 | 580 | 含双早 | 5月1日-10月31日 |
```
"""
