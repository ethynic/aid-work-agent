...
- [x] 1.4 单元测试: 验证 `@应用 消息` → `消息`, `@应用   消息` → `消息`, `普通消息` 不变
...
- [x] 2.5 单元测试: 验证 subscribe 事件触发欢迎消息发送
...
- [x] 5.5 单元测试: 验证写入和读取的 round-trip
...
- [x] 6.4 单元测试: 验证正确和错误的 ToUserName

## 7. 集成测试与回归

- [x] 7.1 运行现有渠道相关单元测试，确保无回归（74 个全部通过）
- [ ] 7.2 手动测试: 私聊文本消息正常收发（需真实 WeCom 环境）
- [ ] 7.3 手动测试: 群聊 @应用 消息前缀已清洗（需真实 WeCom 环境）
- [ ] 7.4 手动测试: 新用户首次进入收到欢迎消息（需真实 WeCom 环境）
- [ ] 7.5 手动测试: 超过速率限制后消息被节流（需真实 WeCom 环境）
