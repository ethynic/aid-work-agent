/**
 * 数字员工 API（只读）
 */
import { getAuthHeader } from '@/api/auth'

const API_BASE = `${import.meta.env.VITE_API_BASE_URL || '/api'}/subagents`

async function handleResponse<T>(response: Response): Promise<T> {
  const data = await response.json()
  if (!response.ok) {
    throw new Error(data.error || `请求失败: ${response.status}`)
  }
  return data
}

// ============== 类型定义 ==============

export interface BusinessPage {
  id: string
  title: string
  icon: string
  route: string
}

export interface SubagentListItem {
  agent_id: string
  name: string
  description: string
  type: 'builtin' | 'custom'
  business_pages?: BusinessPage[]
  // 实例相关字段（租户模式下返回实例时包含）
  instance_id?: string      // 实例ID，租户模式下必填
  display_name?: string     // 显示名称
  instance_name?: string    // 实例名称（如"外贸获客智能体 - 实例1"）
  subagent_type?: string    // 子智能体类型
  status?: string           // 实例状态：idle/busy
  avatar?: string           // 头像
  [key: string]: any        // 允许其他字段
}

export interface SubagentDetail {
  agent_id: string
  name: string
  description: string
  version: string
  author: string
  triggers: Record<string, any>
  tools: Record<string, any>
  skills: Record<string, any>
  context: Record<string, any>
  system_prompt: string
  type: 'builtin' | 'custom'
  business_pages?: BusinessPage[]
}

// ============== API 方法（只读） ==============

/**
 * 获取所有数字员工列表
 */
export async function listSubagents(): Promise<{ success: boolean; data: SubagentListItem[]; error?: string }> {
  const response = await fetch(`${API_BASE}`, {
    headers: getAuthHeader(),
  })
  return handleResponse(response)
}

/**
 * 获取单个数字员工详情
 */
export async function getSubagentDetail(agentId: string): Promise<{ success: boolean; data: SubagentDetail; error?: string }> {
  const response = await fetch(`${API_BASE}/${encodeURIComponent(agentId)}`, {
    headers: getAuthHeader(),
  })
  return handleResponse(response)
}

/**
 * 获取 SUBAGENT.md 原始内容
 */
export async function getSubagentContent(agentId: string): Promise<{ success: boolean; data: string; error?: string }> {
  const response = await fetch(`${API_BASE}/${encodeURIComponent(agentId)}/content`, {
    headers: getAuthHeader(),
  })
  return handleResponse(response)
}

/**
 * 获取可选 skills 列表
 */
export async function listAvailableSkills(): Promise<{ success: boolean; data: string[]; error?: string }> {
  const response = await fetch(`${API_BASE}/skills`, {
    headers: getAuthHeader(),
  })
  return handleResponse(response)
}

/**
 * 获取可选 tools 列表
 */
export async function listAvailableTools(): Promise<{
  success: boolean
  data: Array<{ name: string; description: string; display_name: string }>
  error?: string
}> {
  const response = await fetch(`${API_BASE}/tools`, {
    headers: getAuthHeader(),
  })
  return handleResponse(response)
}

// ============== 租户定制 extra_md（Phase 4 起 DB 驱动） ==============
// 后端：src/api/subagent_extra.py，路径 /api/v1/subagents/{name}/extra

const EXTRA_API_BASE = `${import.meta.env.VITE_API_BASE_URL || '/api'}/v1/subagents`

/**
 * 获取当前租户的 extra.md 内容（DB production 版本）
 */
export async function getExtraMd(
  subagentName: string,
): Promise<{ success: boolean; content: string | null; message?: string; version?: number }> {
  const response = await fetch(`${EXTRA_API_BASE}/${subagentName}/extra`, {
    headers: getAuthHeader(),
  })
  return handleResponse(response)
}

/**
 * 保存 extra.md 内容（提交新版本 + 标记 production）
 * 返回新版本号
 */
export async function saveExtraMd(
  subagentName: string,
  content: string,
): Promise<{ success: boolean; message: string; version?: number }> {
  const response = await fetch(`${EXTRA_API_BASE}/${subagentName}/extra`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json', ...getAuthHeader() },
    body: JSON.stringify({ content }),
  })
  return handleResponse(response)
}

/**
 * 删除 extra.md（级联删除 prompt 记录，恢复默认配置）
 */
export async function deleteExtraMd(
  subagentName: string,
): Promise<{ success: boolean; message: string }> {
  const response = await fetch(`${EXTRA_API_BASE}/${subagentName}/extra`, {
    method: 'DELETE',
    headers: getAuthHeader(),
  })
  return handleResponse(response)
}
