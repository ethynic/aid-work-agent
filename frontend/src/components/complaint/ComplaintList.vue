    ...
    <div class="px-6 pt-6 pb-4">
      <div class="flex items-center justify-between mb-6">
        <div>
          ...
          <p class="text-sm text-muted mt-0.5">投诉全生命周期管理</p>
        </div>
      </div>
    ...
    <div class="px-6 pb-4">
      <div class="flex items-center gap-3 p-3 rounded-xl bg-canvas border border-default">
        <select
          ...
          class="text-sm rounded-lg border border-default bg-surface text-default px-3 py-1.5 focus:ring-1 focus:ring-primary-500 focus:border-primary-500 outline-none"
        >
        ...
        <div class="relative flex-1 max-w-xs">
          ...
          <input
            ...
            class="w-full text-sm rounded-lg border border-default bg-surface text-default pl-9 pr-3 py-1.5 focus:ring-1 focus:ring-primary-500 focus:border-primary-500 outline-none placeholder:text-muted"
    ...
    <div class="px-6 pb-6">
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-default bg-surface/50">
              ...
              <th class="text-left px-4 py-3 font-medium text-muted text-xs uppercase tracking-wider">投诉ID</th>
              <th class="text-left px-4 py-3 font-medium text-muted text-xs uppercase tracking-wider">分类</th>
              <th class="text-left px-4 py-3 font-medium text-muted text-xs uppercase tracking-wider">紧急程度</th>
              <th class="text-left px-4 py-3 font-medium text-muted text-xs uppercase tracking-wider">状态</th>
              <th class="text-left px-4 py-3 font-medium text-muted text-xs uppercase tracking-wider">情绪</th>
              <th class="text-left px-4 py-3 font-medium text-muted text-xs uppercase tracking-wider">投诉内容</th>
              <th class="text-left px-4 py-3 font-medium text-muted text-xs uppercase tracking-wider">交互数</th>
          ...
          <tbody>
            ...
            >
              <td class="px-4 py-3 text-muted tabular-nums">{{ (page - 1) * pageSize + index + 1 }}</td>
              <td class="px-4 py-3 font-mono text-xs text-muted">{{ item.complaint_id }}</td>
              <td class="px-4 py-3">
                <div class="font-medium text-default">{{ categoryLabels[item.category] || item.category }}</div>
                <div v-if="item.sub_category" class="text-xs text-muted mt-0.5">{{ item.sub_category }}</div>
              </td>
              <td class="px-4 py-3">
              ...
              <td class="px-4 py-3 text-muted max-w-[200px] truncate">
                {{ item.description_short || item.description }}
              </td>
              <td class="px-4 py-3 text-center tabular-nums text-muted">{{ item.interaction_count || 0 }}</td>
              <td class="px-4 py-3 text-muted text-xs tabular-nums">{{ formatDate(item.created_at) }}</td>
      ...
      <div v-if="totalPages > 1" class="flex items-center justify-between mt-4">
        <p class="text-sm text-muted">
          共 <span class="font-medium text-muted">{{ total }}</span> 条投诉
        </p>
          ...
          <div class="p-6 space-y-6">
            ...
            <div>
              ...
              <div class="grid grid-cols-2 gap-3 text-sm">
                ...
                <div><span class="text-muted">分类：</span><span class="text-default">{{ categoryLabels[detail.category] || detail.category }}</span></div>
                ...
                <div v-if="detail.customer_emotion"><span class="text-muted">客户情绪：</span><span :class="emotionClass(detail.customer_emotion)">{{ emotionLabels[detail.customer_emotion] || detail.customer_emotion }}</span></div>
            ...
            <div>
              ...
              <p class="text-sm text-muted bg-surface/50 rounded-lg p-3 border border-default">{{ detail.description }}</p>
            </div>
