<template>
  <a
    ...
    class="inline-flex items-center gap-3 p-3 rounded-lg border border-gray-200 bg-gray-50
           ...
           w-full md:w-auto md:min-w-[200px] md:max-w-[280px]"
  >
    <div
    ...
    <div class="flex-1 min-w-0">
      <div class="text-sm font-medium text-gray-800 truncate group-hover:text-gray-900">
...
const mime = computed(() => props.attachment.mime_type || '')

const iconEmoji = computed(() => {
  const name = props.attachment.file_name.toLowerCase()
  ...
  if (name.endsWith('.mp3') || name.endsWith('.wav') || name.endsWith('.amr')) return '🎵'
