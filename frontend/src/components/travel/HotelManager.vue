<template>
  <div class="manager-container">
    ...
    <!-- 统计信息 -->
    <div class="kb-stats">
      <span v-if="!searched">共 {{ allHotels.length }} 家酒店</span>
      <span v-else>搜索结果：{{ searchResults.length }} 家</span>
    </div>
    ...
    <!-- 空状态 -->
    <div v-else-if="currentList.length === 0 && searched" class="kb-empty">未找到匹配的酒店</div>

    <!-- 酒店列表 -->
    <div v-for="hotel in currentList" :key="hotel.doc_id" class="kb-card">
      <div class="kb-card-header">
        <div>
          <div class="kb-card-title">{{ hotel.title }}</div>
          <div class="kb-card-meta">
            <span v-if="hotel.metadata?.sub_region" class="meta-tag">{{ hotel.metadata.sub_region }}</span>
            <span v-if="hotel.metadata?.diamond_level" class="meta-tag">{{ hotel.metadata.diamond_level }}</span>
            <span v-if="hotel.score != null" class="meta-score">相似度: {{ (hotel.score * 100).toFixed(1) }}%</span>
          </div>
        </div>
        <button class="btn-sm" @click="showDetail(hotel.doc_id)">查看详情</button>
      </div>
      <div v-if="hotel.source_file" class="kb-card-source">
        来源文件：{{ hotel.source_file }}
      </div>
      <div v-if="hotel.info" class="kb-card-snippet">{{ hotel.info }}</div>
    </div>
...
import { searchHotelsKB, listHotelsKB, getHotelKB } from '@/api/travelQuote'
...
const currentList = computed(() => searched.value ? searchResults.value : allHotels.value)

async function loadAll() {
  loading.value = true
  try {
    const result = await listHotelsKB({ limit: 500 })
    allHotels.value = result.items || []
  } catch (e) {
