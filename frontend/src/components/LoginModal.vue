<template>
  <Teleport to="body">
    <div v-if="visible" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <!-- Modal -->
      <div class="relative bg-slate-800 rounded-2xl shadow-2xl w-full max-w-4xl mx-4 overflow-hidden">
        <div class="flex flex-col md:flex-row min-h-[500px]">
          ...
          <div class="w-full md:w-1/2 p-8">
            ...
            <div v-if="currentTab === 'password'">
              <div class="space-y-4">
                <div>
                  <label class="block text-sm text-slate-300 mb-1">手机号</label>
                  <input
                    v-model="phoneForm.phone"
                    type="tel"
                    placeholder="请输入手机号"
                    class="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-slate-400 focus:outline-none focus:border-cyan-400"
                  />
                </div>
                <div>
            ...
            <div v-if="currentTab === 'code'">
              <div class="space-y-4">
                ...
                <div>
                  <label class="block text-sm text-slate-300 mb-1">验证码</label>
                  <div class="flex gap-2">
                    <input
                      ...
                      placeholder="请输入验证码"
                      maxlength="6"
                      class="flex-1 px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-slate-400 focus:outline-none focus:border-cyan-400"
                      @keyup.enter="handleCodeLogin"
                    />
                    <button
                      @click="handleSendCode"
                      :disabled="codeCooldown > 0 || isSendingCode"
                      class="px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-cyan-400 hover:bg-slate-600 disabled:text-slate-500 transition-colors whitespace-nowrap"
                    >
                      {{ codeCooldown > 0 ? `${codeCooldown}s` : (isSendingCode ? '发送中...' : '获取验证码') }}
                    </button>
                  </div>
                  <p class="text-xs text-slate-500 mt-1">测试环境验证码固定为：888888</p>
                </div>
                <button
