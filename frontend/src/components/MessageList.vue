<template>
  <div class="h-full overflow-y-auto p-4" ref="containerRef">
    <div class="max-w-4xl mx-auto space-y-6">
      <!-- Empty State -->
      <div v-if="messages.length === 0" class="flex flex-col items-center justify-center h-full text-center">
        <div class="w-20 h-20 rounded-2xl bg-gradient-to-br from-cyan-500/20 to-blue-600/20 flex items-center justify-center mb-6">
          <svg class="w-10 h-10 text-cyan-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
          </svg>
        </div>
        <h3 class="text-xl font-medium text-slate-200 mb-2">开始新对话</h3>
      ...
      <template v-else>
        ...
        <!-- Typing Indicator -->
        <div v-if="isProcessing && messages[messages.length - 1]?.role === 'user'" class="flex gap-3 p-4 rounded-2xl bg-slate-800/50">
