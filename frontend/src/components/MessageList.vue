<template>
  <div class="h-full overflow-y-auto p-4 bg-slate-50" ref="containerRef">
    <div class="max-w-4xl mx-auto space-y-6">
      <!-- Empty State -->
      <div v-if="messages.length === 0" class="flex flex-col items-center justify-center h-full text-center">
        <div class="w-20 h-20 rounded-2xl bg-gradient-to-br from-cyan-100 to-blue-100 flex items-center justify-center mb-6">
        ...
        <h3 class="text-xl font-medium text-slate-700 mb-2">开始新对话</h3>
      ...
      <template v-else>
        ...
        <!-- Typing Indicator -->
        <div v-if="isProcessing && messages[messages.length - 1]?.role === 'user'" class="flex gap-3 p-4 rounded-2xl bg-white border border-slate-200">
