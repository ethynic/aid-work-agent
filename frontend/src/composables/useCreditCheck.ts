...
export function useCreditCheck() {
   ...
   * 行为约定：
   * - 余额 ≤ 0：
   *   - sendMessage：toast.error 阻断，返回 { allowed: false }
   *   - login / newSession：toast.error 提醒，返回 { allowed: true }（不阻断登录/建会话本身）
  ...
  async function checkCreditBeforeAction(action: CreditCheckAction): Promise<CreditCheckResult> {
    ...
    if (creditBalance <= 0) {
      ...
      // login / newSession 仅提醒，不阻断（避免用户连登录都进不去）
      toast.error(blockMsg)
