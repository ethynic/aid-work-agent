...
- 本智能体一级菜单显示为「社媒运营智能体」（来自 `name` 字段），二级菜单「视频制作工作台」（来自 `business_pages[0].title`）跳转到工作台 `/social-media`（MVP 原型，表单式操作：选场景/上传素材/填文案）
- 视频内容的会话化生成（精修/敏捷双模、提示词引擎、企业组织沉淀）由 video-agent 承担，其一级菜单「视频创作智能体」下含素材库/视频库/提示词库三个二级菜单（主聊天流驱动）
