...
**提取规则**：
1. 从上传的文件名（不含路径和扩展名）中，取第一个下划线 `_` 之前的部分作为合同编号候选
...
**重要：当 verify 返回 result=yes 时，你 MUST 立即继续执行步骤2，这是必须的，不是可选的。不要在步骤1通过后就直接输出结果并结束，必须继续执行步骤2完成OA审批。**
...
**result = "yes" 时**：
- 输出校验通过信息
- **必须继续执行步骤2（OA审批）**，这不是可选项
- 审批完成后输出最终结果
- 完整流程 = 步骤1通过 + 步骤2审批，缺一不可

### 必要参数
- **合同编号（contract_code）**：首先尝试从上传文件名中自动提取（见"合同编号自动提取"章节）。如果无法提取或不确定，使用 `clarify` 工具询问用户。NEVER 自行编造合同编号。
- **合同PDF文件路径（file_path）**：从用户上传的文件路径中获取
