...
| 23 | 「为你推荐」区块插入致打招呼计数校验误报 | 2026-08-27 真机：定向点击高长磊实际成功（按钮翻转），但页面同时插入「为你推荐」区块新增 2 个打招呼按钮，总数 6→7 不降反升，旧「总数必须减少」校验误报 EXECUTION_UNKNOWN 中止（滚动懒加载同理）。修复（GreetExecutor）：定向模式彻底放弃计数，按目标验证——after 快照任一可见按钮仍配对出目标姓名才算失败；非定向放宽为「计数减少 \| 点击位置附近(|Δy|≤40,\|Δx|≤100)出现继续沟通」任一。附带坑修复：findButtonsByExactText 遍历同文案全部 strings 下标（原 findIndex 只取第一个会漏按钮）。真机复验：杨鸿杰+李嘉翔（含滚动找人+推荐区块共存）一次通过 |
