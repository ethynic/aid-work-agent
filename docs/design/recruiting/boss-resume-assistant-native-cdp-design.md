...
- **解析所有职位（关键方法修正）**：遍历 `documents[].nodes.nodeValue`（**所有节点，含无 layout bounds 的文本节点**，不只 `layout.nodeIndex`）找「职位 _ 城市 薪资」格式字符串，**职位名 = "_" 前部分**（`PHP开发工程师 _ 上海 8-12K` → `PHP开发工程师`）。真机解析出 4 个职位。
...
- **list-jobs / select-job 设计**：用户传的 job_name 可能不精确（如「PHP」想切「PHP开发工程师」），匹配判断放调用方 AI：① `list-jobs`（只读）打开下拉→等待→解析返回职位列表（"_"前部分 + 点击坐标），AI 据此判断 job_name 匹配哪个；② `select-job`（写动作）由 AI 判断后调用，Win32 点击指定职位坐标切换 + 校验。AI 匹配不到时自然回复用户「职位在 BOSS 中不存在」+ 列出可用职位。
- **不做的事**：CLI 内部不做模糊匹配（判断交给 AI）；不缓存职位坐标（每次 list-jobs 重新打开下拉解析，页面动态）。
