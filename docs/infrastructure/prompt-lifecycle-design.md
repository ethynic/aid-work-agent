...
> **范围调整（2026-07-14，Phase 4 落地时）**：原设计要求编辑器含完整版本管理（历史面板/diff/回滚/草稿）。实际落地时简化为**仅"编辑 + 保存 + 清空"三件事**。原因：extra_md 是租户级**追加内容**（不是 system prompt 主体），通常只是几段定制需求，版本管理价值低；租户改错直接重新编辑即可，不需要回滚到上周的版本。
>
> 后端 `PromptRegistryService` 已具备版本闭环能力（`list_versions` / `diff_versions` / `set_label` 把 production 指向旧版本即等价回滚），如未来真有需求可补充 `GET /extra/versions` 端点 + 前端面板，无需后端改造。
>
> 简化后的保存逻辑：每次 PUT 自动 `commit_version` + 标记 production（tenant_extra 非高危不走 staging）。版本号会自增，但前端不展示历史。

**编辑界面功能（简化版）**：

1. **Markdown 编辑器**：编辑租户定制 Prompt 内容（对应 extra_md 内容）
2. **保存按钮**：提交新版本并标记 production（SHA-256 去重，内容相同不产生新版本）
3. **清空按钮**：级联删除 prompt 记录，恢复默认配置
4. **dirty 检测**：编辑未保存时离开页面有 `beforeunload` 提示
5. **说明卡片**：提示"内容会追加到 system_prompt 末尾"
