...
### 任务 6.2：性能调优与回归
- [ ] 同步压缩耗时 P95 < 30s（含摘要 LLM）
- [ ] 硬截断降级耗时 P95 < 200ms
- [ ] 同一 session 不会出现两次压缩叠加（active summary 唯一性 + DB 部分唯一索引）
- [x] 单元测试覆盖（当前 350 个测试全过）
- [x] 现有所有测试零回归

### 任务 6.3：文档与登记
- [x] 更新 [memory_design.md](./memory_design.md) Phase 2.2 状态为 🔧 部分完成（同步路径已实现）
- [x] 在 `docs/ideas.md` 更新状态为 🔧 部分完成
- [ ] 全渠道联调通过后移动到 `docs/ideas_finished.md` 并标记 ✅ 已完成

**Phase 6 验收**：5 渠道全部联调通过、性能指标达标、文档登记完整。
