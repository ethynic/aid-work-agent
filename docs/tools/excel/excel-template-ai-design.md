...
| `unused_data_keys`（可选） | 用户提供了但模板没有对应位置的数据键（这些**不会被填**，避免硬塞到错列）。agent 据此提示用户"这些内容没填进去"。 |
| `missing_fields`（可选） | 模板里要填但用户没给值的字段（这些**会被清空**，不残留样例示例值）。agent 据此提示用户"模板还需要 X、Y"，便于补全。 |
| `message` | 人类可读的一句话摘要（含上述两类提示），便于 agent 直接转述给用户。 |
...
2. AI 结构分析（数据感知）：
   ...
   > **meta 字段左右结构（重要）**：顶部 meta 是"标题在左、值在右"的左右结构（如 `甲方 | <值>`），
   ...
   > **所有写值一律经 `_set_value` 落到合并区锚点**（左上格）：合并区的非锚点格 `.value` 只读，直接写会报 `'MergedCell' object attribute 'value' is read-only`；且值必须落锚点否则合并后显示锚点旧值。渲染顺序：**只解除"明细区及以下"的合并**（首分组明细首行之上的标题/meta 合并若没内容填，一律不碰，避免被解除后没还原）→ 增删行 → 在**未合并态**写所有值（经 `_set_value` 落锚点，查全部合并定位锚点）→ 最后只把"被解除过的"合并重新应用。
   > **竖向同值合并**：样例明细区内若某列有竖向合并（单列跨多行，如分组类别列 A4:A5 合并居中表示同组），表示"该列同组值相同要合并居中显示"。这类合并不照搬（行数变化无法平移），改为**填充后按相邻相同值重新合并居中**（`_apply_vertical_merges`，只在组内、不跨组、空值不合）。
   > **竖向合并列样式归一**：样例里竖向合并的锚点格（每组首行）是粗体/居中的"类别样式"，续行格是默认样式（合并后不可见）。填充时数据值可能落到续行格上，导致同列类别值样式不一致（有的粗体有的不粗体）。`_normalize_vmerge_styles` 在增删行前把竖向合并列的每个明细格统一复制"该列首个竖向合并锚点"的样式，保证整列一致。
   > **横向单行合并（每行版式模式）**：样例明细区内若某行有横向合并（单行跨多列，如备注列 G4:J4），表示"该范围在每一明细行都要跨列合并"。明细行数随数据增删变化（M>K 会插入新行），无法逐个平移，改为**填充后按列模式（min_col,max_col）应用到每个最终明细行**（`_apply_detail_row_merges`，含新插入行，保证每行版式一致）。此前这类合并被当作"明细区内合并"一并丢弃，导致每行备注只剩 G 列、长文本被截断、版式崩掉。
