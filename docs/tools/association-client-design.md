...
客户端与客户租户绑定，消耗的积分按 **25 倍系数** 计入该租户的通用积分池。
...
| 25倍系数含义 | **实际扣租户积分 = 标准积分 × 25** | 客户端消耗直接乘以25后扣减租户余额 |
...
| `src/services/billing.py`（积分计算） | **服务端使用**，代理端点调用，增加25倍系数 |
...
> **注意**：积分扣减仍然走现有 `tenants.credit_balance`（同事务原子扣减，复用 `ChatRecordDB` 的扣费逻辑或新增平行的 `ClientUsageLogDB`）。`source_type = 'client'` 用于区分。`raw_credit_cost` 记录标准成本（便于审计），`credit_cost` 记录实扣（×25后）。
...
class ClientConfig(BaseModel):
    """客户端场景配置"""
    credit_multiplier: float = 25.0  # 积分膨胀系数，客户端消耗 = 标准积分 × 此系数
...
4. 步骤1：search_profile（文心联网采集原文 → DeepSeek 解析）
   ...
   - ProxyLLMGateway.chat() → POST /api/client/v1/llm/chat
     → 服务端 llm_gateway(DeepSeek) 按原文解析成结构化字段 → 扣积分（×25）→ 返回
...
| `src/api/client_llm_proxy.py` | LLM 代理逻辑（计费 ×25） |
...
| `tests/unit/api/test_client_llm_proxy.py` | 计费单测（验证 ×25） |
