...
## 2026-08-24：官网领导页第一优先 + LLM 逐层导航搜索

秘书长的姓名以**官网为权威来源**（文心大部分正确但存在过期/换届/张冠李戴，
需要官网纠错——真机案例：中国艺术摄影学会秘书长王建琪（胡艳为代秘书长兼
办公室主任）、中国人口学会庄亚儿（文心给了查无此人的魏云鹏）、中国电力
发展促进会换届后张盛勇）。本轮三项改动：

1. **词表扩容**（准入门禁 + 权重两处同步）：补「学会」全套镜像（学会简介/
   关于学会/学会领导——人口学会/冶金教育学会等站点曾被「协会」前缀词表
   整体拦截）、领导泛化词（现任领导/理事长/负责人/理事会/秘书长…提到
   120 最高权重）、机构泛化词（办事机构/职能部门/内设机构/部门设置…）、
   裸词兜底（领导/秘书/理事/概况/简介/章程）。
2. **采集预算放宽**：`collect_official_profile` max_pages 4→8、导航尝试
   12→20（90s 总闸不变），深层领导页不再被预算挤掉。
3. **LLM 逐层导航搜索**（新模块 `official_site_leadership_search.py`）：
   1 万家协会对领导页有几百种叫法，词表列不完；当词表采集没拿到秘书长
   姓名时，把当前页导航栏列表交 LLM 挑「最可能通向领导/组织机构页」的
   链接，逐层点击（最多 4 层/8 次点击/60s），页面内容出现强领导标记
   （秘书长/秘书处/领导班子/驻会/理事长）即收录为领导页，再跑领导提取。
   会长/负责人/理事单独出现不算（会长单位误报），两个弱标记同时出现才算。
4. **领导提取提示词强化**：现任判定（换届/历任/名誉不取）、代秘书长规则
   （无正式秘书长时代秘书长算；同时存在时正式者填秘书长）、兼职秘书长
   正常提取、一人兼多职按实际职务填对应字段（胡艳场景：填办公室负责人）。

验收：`scripts/probe-leadership-search.py`——30 协会真机探针（文心要 URL →
跑生产同款 Step2 → 报告秘书长命中数/LLM 搜索触发数/找到的领导页），
报告增量写 `probe-output/leadership-report.json` 支持断点续跑。

## 2026-08-24 晚：30 协会真机验收（9 轮迭代）

29 家实测：秘书长命中 18/29（62%），从首轮 13 逐轮修复爬升。九轮迭代修掉的真机缺陷
（全部有单测锁定，126 全绿）：

1. 学会词表镜像/领导泛化词/裸词兜底 + 新闻噪声词零分（「相关负责人答记者问」新闻曾拿满权重）
2. 领导页误判链：理事长新闻标题 → 弱标记子串去重；页脚「秘书处」→ 降弱标记；
   首页新闻「秘书长」填满收录名额 → 停止条件与收录解耦 + 返回按强标记密度排序
3. 悬浮菜单站（Element UI/hash 路由/div 导航）：隐藏同域锚点纳入 + URL 直开 +
   导航选择器扩到 nav/menu 容器的叶子 div/span + 命中页继续下钻
4. 慢 SPA：加载占位文案检测轮询窗口 3s→9s
5. 提取层：LLM 输出 max_tokens 800→2000（截断致 STRICT_JSON_INVALID）；
   领导页按标题命中优先排序；主提取异常不再拖垮 Step2
6. 广告外链预过滤（焊接协会首页 40+「查看详情」曾挤爆 LLM 候选）

## 2026-08-24 深夜：人口学会最后一公里（5 个根因连环修复，19/29）

中国人口学会（cpaw.org.cn，秘书长庄亚儿在 现任领导 第 3 页）从 ❌ 到 ✅，连续 6 轮探针
逐根因修复。**这 5 个根因全部是通用缺陷，波及所有协会的 LLM 搜索链路**：

1. **思考链烧穿 max_tokens（最深根因）**：deepseek-v4-pro 带思考链，`_llm_pick_navigation`
   的 max_tokens=300 被思考烧光 → content 恒为空 → `via=deterministic` 兜底。回看历史
   报告发现**循环经济协会那次成功也是 deterministic 兜底命中的——LLM 逐层导航从未真正
   生效过**。修复：协会链路 5 处小 max_tokens 全部放大（nav-pick 300→4000 等，按生成量
   计费，上限只是保险丝）。
2. **换模型 + 关思考**：模型 deepseek-v4-pro → deepseek-v4-flash（.env，更便宜），且
   DeepSeekProvider 默认注入官方关思考参数 `thinking: {"type": "disabled"}`（v4-flash
   与 v4-pro 均实测生效）。注意 `chat_template_kwargs` 是 vLLM 本地部署用法，官方 API
   不识别（实测属软提示、不保证生效，曾在 v4-pro 上烧 2000 token 输出为空）。关思考后
   nav-pick 从 20~95s/3500 token → **0.6~1.6s/9 token**。搜索时限 120s→300s（思考
   关掉后裕量充足）。
3. **BFS 浏览器漂移失配**：`activate` 在「当前 DOM」按 text+href 匹配元素，但 BFS 出队
   的页与浏览器实际位置失配（同轮第 2 个候选/兄弟分支的点击已把浏览器带走），在错误的
   页上匹配不到侧栏目标 → 静默 None → 8 次点击预算全耗光。修复：跟踪 `browser_state`，
   activate 前失配则重开目标页 URL 回位（词表采集「重放官网入口」同款；direct_url 直开
   不需要回位）。
4. **flip_next 把「尚未变化」误判为「已稳定」**：点击下一页后只等 800ms 就进稳定轮询，
   慢 SPA 渲染延迟 2~3s，内容还没换就因指纹未变放弃翻页 → 第 3 页（庄亚儿）永远翻不到。
   修复：先等「内容开始变化」（4s 窗口）再等稳定，并补 el-loading-mask 遮罩等待；新增
   `leadership_flip_stop` 审计事件记录翻页终止原因。
5. **收录名额与排序挤掉真名单页**：强标记密度并列（都算 2）时按插入序取前 3，翻页尾部
   的名单页被挤掉。修复：排序改秘书长出现次数优先；`MAX_FOUND_PAGES` 3→5；搜索页
   全量交给提取器。

修复后全链路：文心给 URL → 词表采集 → LLM 选航（关于学会→现任领导）→ activate 回位
点击 → 翻页到第 3 页 → 秘书长=庄亚儿 ✅。221 测试全绿。剩余 10 家失败归因不变：4 家
URL/可达性源头问题；1 家官网无领导页（焊接）；5 家特殊结构待用户提供领导页位置后
针对性适配。

## 2026-08-25：剩余 10 家复验（新链路再转化 4 家，23/29）

用修复后的链路复验全部 10 家失败案例：**家纺（王易）、老年保健（王骋）、退役军人
（袁英敏）无需任何适配直接转命中**；焊接协会按用户指引（秘书长在 关于协会→发展历程）
连环修 4 个通用根因后转命中（李连胜）：

1. **发展历程类页面不在选航词表**：选航提示词与确定性词表都没有「历程/大事记/沿革」，
   LLM 在焊接协会「关于协会」子菜单永远挑 协会介绍/组织架构、跳过发展历程。修复：
   两处词表加词（确定性 80 分档），提示词说明此类页面常载换届记录。
2. **每节点只点前 2 候选**：发展历程排 LLM 候选第 3 位永远轮不到。修复：picks[:3] +
   点击预算 8→10（点回已访问页会被 fingerprint 快速跳过，浪费有限）。
3. **悬浮子菜单重载即收起**：焊接协会「关于协会」子菜单 hover 展开，BFS 回位重开
   首页后子菜单收起，activate 在可见元素里匹配不到目标静默失败。修复：activate 无
   可见匹配时逐个 hover 主导航项展开子菜单再找（`_hover_sweep_for`，模拟真人）。
4. **裸词排序被新闻刷词压过 + 换届页被提取规则误杀**：新闻标题「秘书长工作会议」×3
   把名单页（「秘书长为李连胜」×1）挤到榜尾；提取规则①「换届/历任不取」会把发展
   历程里最新一届的选举记录也拒掉。修复：排序改「秘书长-姓名绑定」计数优先
   （`秘书长[为：:]X`，新闻零绑定）；规则①澄清「发展历程最新一次选举记录=现任，
   更早年份才是前任」。

剩余 6 家：冶金教育学会（慢 SPA 菜单 8s+）、残疾人康复协会（全站同标题 SPA）待诊断
或用户提供位置；iamac/野生动物（站点超时）、针灸（死链等正确 URL）、青年科技（文心
无 URL）为可达性/源头问题。

### 2026-08-25（续）：冶金教育学会（用户指引：关于学会→学会领导，24/29）

用户指路后手动复现发现两个通用缺陷（修复后词表采集直接命中，LLM 搜索都没触发）：

1. **open() 在菜单渐进渲染时提前返回壳状态**：轮询条件是「内容非空 + 导航非空 +
   非占位文案」，csmedu 首轮就有 1378 字正文 + 1 个导航项即满足返回，而菜单
   1→8 个目标要 3s+ 才渲染完——nav_probe 拿到 targets=1 的壳，点击预算全浪费。
   修复：导航数量仍在增长时继续等，连续两轮一致才返回（快站仅多 250ms）；
   goto 超时恢复路径保持「可读即返回」原契约。
2. **绑定正则缺「职务行+姓名行」模式**：csmedu 学会领导页是
   `秘书长\n\n孙建林（兼）`（职务/姓名分行，兼秘书长），`秘书长[为：:]X`
   匹配不到 → 排序信号丢失。修复：绑定模式加换行变体
   （`秘书长\n姓名[（兼）]`），并有单测锁定（新闻裸词仍计 0）。

### 2026-08-25（续二）：残疾人康复协会（用户指引：协会概况→基本情况→理事会领导，25/25）

一行修复：**CSS 属性选择器大小写敏感**。该站侧栏容器 class 为「MenuList」
（Vue 驼峰命名），`[class*='menu']` 匹配不上 → 整个侧栏（含「理事会领导」）
从导航探测里消失，nav_probe 只见 1 个目标，点击预算无处可花。修复：
NAVIGATION_SELECTOR 的全部 class 匹配加 CSS `i` 标志（大小写不敏感，
`[class*='menu' i]`）。修复后词表采集直接命中（侧栏可见后词表路径
协会概况→基本情况→理事会领导 全在），秘书长=郭韶华。

**至此 25 家成功跑完采集的协会 25/25 全命中**；剩余 4 家均为环境/源头
...
### 2026-08-25（续四）：客户端每协会余额守卫（防透支/防白跑）

**原缺口**：服务端只在每次 LLM 调用前查余额（≤0 拒绝 402），客户端把 402 的
NoCreditError 当单协会失败吞掉后继续跑下一个协会——后续协会白跑文心浏览器/
微信 RPA 等不计费步骤且无任何提醒；且最后一笔 LLM 可把余额扣成小额负数，
透支范围是整个批次。

**修复**：`AssociationBatchEnricher` 新增可选 `credit_guard`（每协会开始前
调用，抛异常即把当前协会标 `aborted`、批次熔断停止）。客户端注入余额查询
（`GET /api/client/v1/credits`）：余额 ≤0 抛 NoCreditError，进度/遥测明确提示
「充值后重新运行即可继续」，退出码 3；查询失败不阻断（fail-open，402 兜底），
透支上限压到一个协会的消耗内。批次熔断时未处理协会名单在错误提示中列明。

## 2026-08-12：文心秘书长手机号改为 LLM 判别归属

文心联网回答常含**多个**手机号（协会其他人/办公室条目），原「纯正则取第一个」
会张冠李戴。`wenxin_search_secretary_mobile` 改为两段式：正则只负责圈出候选
（1[3-9] 开头 11 位）→ LLM 结合原文上下文判别哪个号码**明确属于该秘书长本人**
（姓名与号码需同段/同条目/有职务绑定表述）→ 只在 LLM 明确绑定时返回，且返回值
必须属于候选集合（防模型幻觉编造号码）。LLM 判不出、判失败、判为非本人均返回
None，落回微信搜一搜兜底——宁缺毋错。

## 2026-08-12：采集联系人角色调整（会长 → 会员服务/办公室负责人）

采集目标联系人从「会长 + 秘书长」调整为「秘书长 + 会员服务相关部门负责人 +
办公室/综合办负责人」三个角色，会长彻底不采。聚焦能联系上的实操对接人。

- `PROFILE_FIELDS` 14 → 16：删除 `president_name`/`president_mobile`，新增
  `member_director_name`/`member_director_mobile`、`office_director_name`/
  `office_director_mobile`；办公室/综合办合并为一个角色字段。
- 文心问句（`QUERY_TMPL`）改查「现任秘书长、会员服务相关部门负责人、办公室或
  综合办负责人」姓名。
- 官网领导页兜底提取（`_extract_leadership`）与领导页筛选词同步换成三个新角色；
  主提取缺任一角色姓名即触发兜底。
- 编排器按 `CONTACT_ROLES` 循环：Step3 微信搜姓名、Step4 查手机号对三个角色走
  完整链路；仅秘书长保留文心手机号快速路径。
- Excel 人员列只保留三个新角色的姓名/手机，会长及其他职务（常务副秘书长、
  学术部主任等占位列）全部删除。
- `wechat_search_leader_name` 删除死参数 `known_president`（无人再传）。
...
官网 16 字段仍分别执行原有来源、quote、格式和姓名—手机号绑定校验，但校验终态改为字段级：
无关计数、邮箱或地址字段不合格时仅清空该字段，已经验证的联系人及其他字段继续保留；
...
联系人姓名使用独立的官网领导页提取，即使主 16 字段提取失败也会运行；三个联系人角色之间同样按字段
...
5. 已知秘书长/会员服务/办公室负责人姓名但官网没有手机时，逐人调用微信搜一搜 RPA；
6. 写一个 XLSX，包含协会名称、16 个业务字段、处理状态、来源摘要、错误摘要和处理时间。
...
- 主管单位、单位等级；
- 秘书长姓名/手机、会员服务负责人姓名/手机、办公室/综合办负责人姓名/手机；
- 单位地址、单位邮箱；
...
| 秘书长/会员服务/办公室负责人 | 组织机构、领导、联系我们、最新通知 | 姓名与职务必须同一证据块 |
| 联系人手机 | 联系我们、通知、附件 | 姓名和号码需同段/同表格/明确字段绑定 |
