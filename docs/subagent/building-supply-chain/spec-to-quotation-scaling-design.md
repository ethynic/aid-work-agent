...
**补充（2026-09-17，WorkBuddy 对标）**：观察到第三方 WorkBuddy 对大文件**事前主动分多文件输出**（按空间部位拆，单文件 3~5MB）。据此确定体积控制优先级：**主动拆分（第 1 步拆分裁决，单文件 3~5MB）> 事后降质兜底**。拆分更优的原因：供应商分包报价更清晰，且不损失图片清晰度。SKILL.md 拆分裁决已加入该规则（预估条目 > 50 或图片 > 30 张时按空间部位再细分）。
