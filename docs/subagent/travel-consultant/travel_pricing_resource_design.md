...
**一行一个价格（六列，第一行为表头）**，格式：`房型 | 散客价 | 团客价 | 含早 | 适用日期 | 备注`

- **房型**：合并床型/面积/景观等属性（如"大床房（1.8米大床·30㎡）"）
- **散客价 / 团客价**：两个独立数字列；只有一种对外价时两列同值
- 只解析对外价格（不解析内部结算价/成本价）；无对外价的房型整行丢弃
- 备注：差价规则（"团散+20"）、"节假日专用"等
...
请整理为以下格式（第一行为表头，之后一行一个房型+日期区间）：
房型 | 散客价 | 团客价 | 是否含早 | 适用日期 | 备注
