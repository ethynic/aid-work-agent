...
格式为表格，**第一行为表头**，之后每行一个房型+日期区间的价格组合：
房型 | 散客价 | 团客价 | 含早 | 适用日期 | 备注

**列含义**：
- **房型**：合并床型/面积/景观等属性的完整房型名（如"大床房（1.8米大床·30㎡）"）
- **散客价**：对外非团队价格（含门市价、参考价、网站价等来源），元/间/夜
- **团客价**：团队对外价格（含团队价、团散同价、协议价等来源），元/间/夜
- **含早**：含双早/含单早/不含早/未知
- **适用日期**：具体日期范围
- **备注**：其他说明（如"团散+20"、"节假日专用"等）

规则：
1. 所有模糊日期（如"五月"、"7-8月"）转换为具体日期范围（如"5月1日-5月31日"）
2. 节假日单独列出行（五一、国庆、端午等），备注列注明"节假日专用"
3. "团散+20"等差价规则 → 团客价 = 散客价 + 差额，直接计算；备注列注明
4. 司陪房、免房政策等特殊规则不进入房型行；如有，附在价格表末尾以"特殊政策："开头
5. 如果某月份列无数据，则跳过该月份
6. **同一房型同一日期区间**散客价/团客价都有时分别填两列；只有一种对外价时**两列都填同值**
7. **只解析对外价格**，不解析"结算价/成本价/底价/给社价/内部价"等内部价
8. **完全没有对外报价的房型整行丢弃**，不进入知识库
9. **必须输出表头行**（`房型 | 散客价 | 团客价 | 含早 | 适用日期 | 备注`），下游解析器依赖表头跳过
