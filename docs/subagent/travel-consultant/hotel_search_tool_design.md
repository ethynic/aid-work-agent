...
命中酒店后，批量查 chunk_index=1 的 `price_table`（避免 N+1），作为 `price_table` 字段返回全文。价格表是 markdown 表（六列：`房型 | 散客价 | 团客价 | 含早 | 适用日期 | 备注`，第一行为表头），体积小，无需截断。
  ...
  "results": [
    {
      ...
      "price_table": "房型 | 散客价 | 团客价 | 含早 | 适用日期 | 备注\n高级城景房型 | 580 | 450 | 含双早 | 5月1日-6月30日 |\n...",
      "score": null,
      "region": "贵阳",
      "diamond_level": "4钻"
    }
