...
> 状态：2026-09-20 v2 修订（设计完成，未开发）。
> v2 修订：客户 API 说明到位（宏陶商城 `hongtaoshop.gzfenxiao.com`，「论坛帖子」+「商城商品」两个只读列表接口，见 §13），接入路线定稿为**文档即配置、agent 驱动、不写每接口代码**；运行期执行模式定为契约直执（默认）+ agent 循环兜底。
...
| 阶段 | 内容 | 状态 | 完成记录 |
|------|------|------|---------|
| Phase 1 | 核心管道：4 表 DDL + 文档上传 + agent 契约生成（读文档 + http_api 实探 + dry-run 样例）+ 契约直执引擎 + 匹配/整理 + 幂等入库 + 手动入口（API + agent 工具） | 📋 待开发 | — |
| Phase 2 | 调度与治理：驱动协程 + 定时同步 + agent_loop 兜底模式 + 失败退避 + stale 回收 + 余额预检/计费 + two_strike 删除 + portal 审计视图 | 📋 待开发 | — |
| Phase 3 | 前端接入向导 + 图片转存/VL（可选，按张计费）+ knowledge 删除联动回写 + 客户存量接入迁移 | 📋 待开发 | — |
...
| D5 | 删除检测 | 仅"完整可靠全量扫描"才触发（分页走完 + `total` 核对一致，接口无 total 时降级为分页耗尽）：默认 `two_strike`（连续两轮全量缺失才软删），可配 `off`/`mark_only` | 对齐公众号"宁可不删不可误删"；本客户接口返回 total，完整性可硬校验 |
...
| D8 | 列表/详情两种形态 | 契约声明 `detail_endpoint` 有无：无则 `list_payload` 模式（列表响应即全量数据，本客户即此形态）；有则 `detail_fetch` 模式（列表只产 external_id + 时间戳，逐条拉详情） | 覆盖常见 ERP/开放平台两种形态 |
| D9 | 与 http_api 工具的代码关系 | **运行期直接复用 HttpApiTool**：在隔离 ToolRegistry 内实例化白名单子类（host 白名单 + 响应大小上限 + 审计），获得其环境变量替换、占位符 fail-fast、业务错误识别、大响应落盘全部现成能力（recap external_push `_create_tool_runtime` 同款装配）；不改共享工具路径 | 用户点名"用现成工具调用接口"；SSRF 债务不外溢——无人值守路径用子类自守白名单，共享工具债务独立登记 |
...
| D13 | 知识粒度（v2.1 新增） | 契约增加 `granularity`：`whole`（一条记录渲染后整条 1 个 chunk，一条记录=一条知识=一个检索单元，产品类默认）/ `chunked`（TextChunker 切分，长文类）/ `auto`（agent 在契约生成期按样例记录渲染后 token 分布决定并写入契约，dry-run 展示依据：p95 token 数与切块数预估，人工可改）。whole 模式单条超 `whole_max_tokens`（默认 2048）时**逐记录**回退 chunked 并记 `metadata.granularity_overflow`，不影响其他记录 | 用户要求智能体有能力决定整条进入还是切分进入。chunks 表本按文档组织，whole 模式天然实现「一产品一条知识」；逐记录回退应对长尾大详情商品；运行期确定性执行契约，agent 不必每 run 决策（与 D1 一致）。knowledge 侧已有 precomputed_chunks 旁路先例，非默认分块不破坏架构 |
...
【接入期 · 一次性，agent 驱动】
  ...
  dry-run 预览（拉 1 页、渲染 ≤3 条样例文档、external_id/hash 预览、字段 in_doc/metadata_only 分配）
        ▼
  人工确认（或高置信度自动通过）→ bs_api_ingest_sources（secrets 加密）→ verify 通过启用
...
【运行期 · 确定性治理，LLM 仅兜底】
  ...
  入库：可选 LLM 摘要 → TextChunker → embedding → 单事务落
       ...
       → 惰性建分类 k_api_ingest_{source_code}
                 ▼
  收尾：run 计数终态 / 计费 fail-open / last_synced_at 与游标推进
```
...
以本客户「商城商品」源为例（§13 有完整核对）：

```jsonc
{
  "mode": "list_payload",                       // list_payload | detail_fetch
  ...
  "granularity": { "mode": "whole",              // whole（一记录=一条知识，产品类默认）| chunked | auto（契约生成期 agent 按样例 token 分布定）
                   "whole_max_tokens": 2048,      // 整条上限，超限逐记录回退 chunked 并记 metadata.granularity_overflow
  ...
  "confidence": 0.95                             // 契约生成期 agent 自评；低于阈值强制人工确认
}
```
...
输入：租户上传的接口文档全文。步骤：LLM 读文档产出候选契约（endpoint/分页/记录路径/字段清单/模板建议/置信度）→ 按契约用隔离 HttpApiTool **实探** 1 页验证（success_check、records_path、native_id 逐项校验）→ dry-run 渲染 ≤3 条样例文档 + hash 预览 + in_doc/metadata_only 分配表 → 低置信度强制人工确认。护栏：LLM 上下文只含文档与样例响应，凭证占位符不渲染；实探域名必须与文档一致且在 allowed_hosts 内；实探只允许 GET/HEAD。契约生成失败 → 允许管理员手改契约 JSON 重跑 dry-run，或源置 `agent_loop` 模式先跑起来。
...
**agent_loop（兜底）**：recap external_push 同款受限循环——lite 模型 + 仅两个工具（白名单 HttpApiTool + submit_records），系统提示嵌文档全文与任务规格（"分页遍历完整列表，逐页提交原始记录"），每轮对工具参数做确定性纠偏（method/固定 query 以契约为准、页码自增、status≠1 即停）；护栏：max_iterations、单 run token 上限、total 核对，不达标 run 判 failed 不入库。适用于契约生成失败但文档清晰的场景。
...
2. `content_hash = sha256(pipeline_version + template_id + in_doc 字段规整序列)`；metadata_only 字段只落 documents.metadata。
3. hash 相同 且 doc active 且 pipeline 相同 → `_handle_check_unchanged`（零计费跳过，仅更新 metadata_only）；doc 被软删 → `_handle_restore`；doc 行缺失 → 重建。
4. 可选 `template+summary`：LLM 生成 ≤500 字摘要（超时重试 1 次，失败回退原文，对齐公众号 summarize）。
5. 按契约 granularity 落块：`whole` 模式整条 1 个 chunk（超 `whole_max_tokens` 逐记录回退 TextChunker 切分并记 `metadata.granularity_overflow`）；`chunked` 模式 TextChunker（chunk_size/overlap 可按契约覆盖）。随后 `_persist_document_tx` 单事务：documents `INSERT ... ON CONFLICT (tenant_id, origin, external_id)` / UPDATE 覆盖 + chunks/chunks_vec 删旧重建 + record/item 终态 + 惰性建分类。metadata 记 `raw_payload`（原始记录 JSON，按契约 max_kb 截断、超出转存租户文件放指针）+ metadata_only 字段值（含 label）+ `granularity/overflow 标记` + 溯源（source_code/native_id/run_id/content_mode/pipeline_version/ingested_at）。whole 模式下一个产品 = 一条知识 = 一个检索单元，检索命中即完整产品，不自我拆分占用多个 top-k 名额。
...
**删除联动（双向）**：全量对账按 D5 保守策略 `active→missing(miss_streak++)→连续两轮→deleted`，同步软删 documents；知识库侧删除 `origin='api_ingest'` 文档时回写 record 软删（`knowledge/service.py delete_document` 增加 origin 分支，Phase 3），防复核付费重建。
...
- 生成器只建 queued run，幂等（NOT EXISTS 活跃同类 run 门禁），错峰上限与公众号一致。
...
- **SSRF**：契约 `allowed_hosts` 域名白名单（契约生成期、保存时、运行期逐请求含重定向后 host 三重校验）；运行期用 HttpApiTool 白名单子类，共享工具的 SSRF 债务不外溢。
- **凭证**：secrets 加密落库；日志/审计/错误信息一律脱敏（复用 wechat_mp `sanitize_error_info` 纪律）；占位符未解析 fail-fast 拒发（继承 http_api Code=-99 事故教训）；契约生成期凭证不进 LLM 上下文。
...
- **Phase 1（核心管道）**：4 表 DDL + config_codec + contract（schema/映射/HTML→markdown/通用结构渲染器/raw_payload 留存/granularity 落块/hash）+ executor（contract_direct + 白名单 HttpApiTool 子类）+ service（受理/处理/入库/收尾）+ assistant（契约生成 + 实探 + dry-run，含 granularity auto 决策）+ 租户端 API + agent 工具两个 + 单测（翻页停止条件/success_check/hash 幂等/HTML 转换/结构渲染器/whole 回退切分/mapping 边界/并发 upsert 冲突）。
- **Phase 2（调度与治理）**：scheduler 驱动协程 + notify + tick 生成器 + 退避/stale 回收 + agent_loop 兜底 + 计费 + two_strike 删除 + portal 审计视图 + 集成测试（fake API server，含本客户接口形态的仿真）。
- **Phase 3（收尾）**：前端接入向导 + 图片转存/VL（可选）+ knowledge delete_document 回写分支 + 本客户两源（论坛帖子/商城商品）正式接入，替代存量脚本。
...
| 全量对账误删（对方接口故障返回空列表） | D5 完整性门禁（total 核对 + 分页耗尽）+ two_strike + `off/mark_only` 逃生开关 |
...
| 知识粒度 | 商品源 `granularity=whole`（一条商品=一条知识，检索命中即完整商品）；dry-run 按样例渲染后 token 分布定，个别超大详情商品逐记录回退切分并标 overflow；帖子源 content 为长文 → agent 在契约生成期决定 chunked 或 auto（dry-run 展示 p95 token 与切块数依据） |
