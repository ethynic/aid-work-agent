...
**中文 tokenizer 效率横向基线（2026-08-16 补充实测，脚本 `/tmp/tok_ds.py`）**：同文本分别请求 deepseek-v4-flash / qwen3.7-flash / mimo-v2.5，对比 `prompt_tokens`（每千字 token 数，越低越省）：

| 文本 | deepseek | qwen3.7-flash | mimo-v2.5 |
|------|---------|--------------|-----------|
| 自然中文 350 字（非重复） | 863.8 | **577.1** | 1465.9 |
| 自然中文 1000 字（重复拼接） | 640.0 | **535.0** | 812.0 |
| 业务审批文本 3800 字 | **612.4** | 620.3 | 714.2 |
| 英文对照 900 字 | 287.4 | **234.8** | 411.1 |

- 中文效率排序：**qwen（优等生）> deepseek（良好）> mimo（差）**。deepseek 非重复中文每千字 864 token，比 qwen（577）贵约 50%（纯中文原创内容形态，对项目影响最大）；但明显优于 mimo（1466）。
- 文本越"结构化"差距越小：业务审批文本（数字/固定句式多）deepseek 与 qwen 几乎打平（612 vs 620），因数字/标点 token 效率各家接近，稀释中文差距。
- **意义**：deepseek 是"良好"而非"优等"，其中文效率即本项目默认 provider 的**原始成本锚点**——以后评估任何替代模型时，凡每千字中文 token 明显高于 ~864 的（如 mimo），应视为中文 tokenizer 劣势并警惕其成本放大。
