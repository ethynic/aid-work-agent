...
| GLM-5.3-Flash | 0.8 | 2.8 | 0.23 | 2026-09-14 补测；cached 0.23 为官方 2026-09-17 公布（补测时暂按 0.8 计价，实测 0 命中，成本数据不受影响） |
...
| 命中折扣 | 0.04 / 2.0 = 2%（无门槛 + 长持久，折扣力度最大） | 0.16 / 0.8 = 20% | cached 0.23 / input 0.8 = 28.75%（2026-09-17 官方公布，此前暂按 0.8 无折扣） |

9-11 实验 qwen 全部 0 命中的原因（已复测澄清）：① doc 2224 仅 726 tokens 低于 1024 门槛，永远不可能命中；② 其余 4 篇关思考→开思考的重复调用仅间隔约 10 秒，缓存尚未构建完成，实测间隔 30 秒后即可命中。生产批量任务（摘要回填等）走 qwen 时，重复同前缀调用间隔拉开数十秒即可享受 20% 缓存价。GLM-5.3-Flash 补测时暂按无折扣计价（cached 0.23 为 2026-09-17 官方公布，晚于补测），0 命中对成本无影响。
...
- GLM 单价与 qwen3.8-flash 接近（input 0.8 vs 0.8，output 2.8 vs 2.7），cached_input 0.23（2026-09-17 官方公布）是 qwen（0.16）的约 1.4 倍，且为唯一多模态通道。
