...
| 49 | AI 批量视频内容生产系统 | 📋 待开发 | **v0.4 回归本质（2026-07-29）**：这不是"批量生产系统"，是一个**抽卡式 AI 视频工具**——选场景→上传素材填文案→生成2-4个视频抽卡→满意留下继续优化编辑→下载。运营拿到成片自己上传视频号/抖音。**MVP唯一闭环**：①选1-2个通用场景(产品展示/氛围)；②向导内上传产品图/模特图(自动抠图,落file_id)+填文案；③一次生成2-4条差异化视频卡片式展示供挑选；④满意的"留用"可重新生成式编辑(换运镜/换背景/改文案重跑,不做帧级剪辑)，最终下载。**v0.4 相对v0.3收敛**：删除审核台、删除发布链路对接(到下载为止)、删除批量任务看板(就是一次抽卡)、场景从7类收到1-2个、编辑改为重新生成式(不集成前端剪辑器)。**极简数据模型**：gen_sessions(一次抽卡会话)+gen_cards(抽出来的视频卡片,含kept留用标记/parent_card精修溯源)，素材复用file_id不建表，无gen_batch_tasks/social_*对接。**内部仍走工艺链**(抠图→主体一致→prompt→运镜→通义万相I2V→拼接→超分→字幕→合规标识)但对用户隐藏为"一次抽卡"。**Spike实测结论(v0.5,2026-07-30)**：模型`wan2.7-r2v-2026-06-12`(r2v)，用`reference_image`+`first_frame`防变形成功(假睫毛/模特图均验证产品不变形)；**万相支持base64直传图**(无需公网URL/OSS)；**长图须人脸裁剪**否则鬼影(新增preprocess预处理环节)；必须用httpx。**合并架构(v0.5)**：视频生成合并到社媒营销智能体作为"内容创作"模块，后端独立模块(src/video_gen/)+社媒工作台内加Tab入口(不进chat,不新增路由,社媒零侵入)，MVP成片先独立下载暂不接审核发布链。**MVP 4-6周**：通义万相API。**Phase1**：更多场景/自建Wan2.2/批量/产品库/接社媒内容管理流。**Phase2**：审核发布链路/帧级剪辑/数据回流。PRD第0章为权威定义，MVP技术设计/开发计划已就绪(spike验证通过)。2026-07-30 | [调研](research/ai-video-production-research.md) / [PRD](system/content-production/ai-video-production-prd.md) / [MVP技术设计](system/content-production/mvp-design.md) / [MVP开发计划](plans/plan-video-gen-mvp.md) | — |
