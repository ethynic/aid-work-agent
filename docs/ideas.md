...
| 34 | PPT 工具增强与 HTML 转 PPTX 导出 | 🔧 部分完成 | 整合现有 PPT 工具设计、深度调研、HTML 转 PPTX 调研和文件工具入参契约 Phase 4。目标：补齐 `instruction/content/content_type/output_name/export_mode` 结构化入参；直接引入 Node.js + PptxGenJS 作为新增 PPTX 主渲染器；自研与 PptxGenJS 高度对齐的 `SlideDeckSpec` 中间协议，并借鉴 Codex Presentations 插件的 layout registry、模板跟随、frame map、渲染 QA 和对象检查机制；支持 HTML 页面 PPT 导出为高保真截图版 PPTX，以及尽量保真的可编辑 PPTX；不纳入 Google Slides。Phase 0-3 已完成：已落地结构化入参、Python schema/planner 转换、Node/PptxGenJS CLI、默认 Node 渲染及回退；新增 11 类 layout registry，Planner 仅输出 layout id，builder 按 registry 确定性布局并对超量内容拆页/截断，warnings 可审计；PPT Python 51 项、Node 5 项测试及 build 通过。HTML 导出仍待 Phase 4。2026-06-30 | [设计](tools/ppt/ppt_tool_enhancement_design.md) | [开发计划](tools/ppt/ppt_tool_enhancement_dev_plan.md) |
