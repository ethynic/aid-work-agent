...
> 2026-07-10 补充：RPA 出站协议升级至 v1.1，`ActionEnvelope.reply_context` 明确携带发送人、原始入站文本、Agent 回复和桌面端会话搜索名；客户端使用去除末尾 `@微信` 的搜索名定位窗口，缺少可靠搜索名时拒绝盲发。服务端补齐已鉴权 config→client→租户命名空间 account 映射、在线/离线 `type=actions` 信封、outbox 上下文持久化和真实 `send_ok`，前端支持维护会话搜索名。状态仍为 🔧 部分完成，待 agent2 真机验证完整下发与执行回执。
