...
| 34 | PPT 工具增强与 HTML 转 PPTX 导出 | 🔧 部分完成 | Phase 0-6 已完成：已落地结构化入参、SlideDeckSpec、Node/PptxGenJS 渲染及回退、11 类 layout registry、HTML 高保真/可编辑双模式；完成模板跟随增强，支持模板尺寸、master/layout、placeholder、字体、颜色和原始页面审计，生成 `template-audit.json`、`template-frame-map.json`、`deviation-log.json` 临时产物；新增页面优先复用源模板 layout/placeholder，显式模板页支持克隆替换，无法安全匹配时记录偏差并返回 warning，不混用内置 theme。真实 Playwright、真实模板、双 PPTX、临时目录清理及 PPTX 重开测试已通过。Phase 7 质量报告及 Phase 8 提示与文档收尾仍待开发。2026-07-01 | [设计](tools/ppt/ppt_tool_enhancement_design.md) | [开发计划](tools/ppt/ppt_tool_enhancement_dev_plan.md) |
