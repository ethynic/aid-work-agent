...
> 2026-07-15 补充：#29 会话绑定名称回填与无效项清理开发中。后续事件获得真实名称时，仅在旧名称仍为空、unknown 或等于稳定标识/搜索键时自动回填，保留人工维护名称；租户页仅允许删除 pending / needs_review / invalid 绑定，active / paused 禁止直接删除，并以租户条件和原子状态条件避免跨租户或并发误删。
...
| 29d | RPA 外部联系人绑定识别与清理 | 🔧 部分完成 | 2026-07-16：RPA 与 `wecom_kf` 完全隔离；仅外部联系人进入 binding/Agent，待确认等无效绑定可安全删除。已新增独立加密字段 `external_contact_secret`，使用同租户 CorpID 调 `externalcontact/get` 解析姓名；首次消息优先以姓名建绑定，管理页每次有界回填 20 条历史占位名，缓存、失败退避、租户与占位条件双重隔离，不覆盖人工名称。外部联系人缺少可靠搜索名时服务端拒绝出站，避免用 wm/wo ID 盲搜误发。待配置生产 Secret 并完成 agent2 真机验收后转完成。 | [姓名解析调研](research/wecom-rpa-external-user-name-resolution-research.md) | — |
