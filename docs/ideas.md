...
| 37 | 图片资产全链路承载能力 | 🔧 部分完成（Phase 0 已完成） | **系统级横切能力**：定义图片资产（Image Asset）从来源/注册/寻址/嵌入/渲染的统一规范。触发场景：旅游顾问行程 Word 缺景点图片。四层架构：① 资源管理（ImageRef + ImageRegistry，复用 cp 的 file_id 体系）② Agent 回复承载（新增 images SSE 事件 + ChatMessage.images + 前端 ImageGallery + 渠道图片消息）③ 知识库图片资产（documents/chunks.metadata JSON 扩展，attraction_search 返回 cover_image）④ 文档生成嵌图（通用 image_inliner 支持 file_id:/kb:// 自定义 scheme，Word/PDF/PPT 共用）。设计原则：复用 cp 基建、来源无关、统一契约 ImageRef。Phase 1 直击旅游顾问，Phase 2 解锁 Agent 回复显图（所有子智能体受益）。与 file_usage.md / cache_usage.md / backend_dev.md / frontend_dev.md 需协调增补。**Phase 0 已完成**（2026-07-13）：ImageRef + ImageRegistry + 15 单测全绿（含 Content-Length 预检回归用例），复用 cp Redis 协议（`uploaded_file:{file_id}`），file_usage.md / cache_usage.md / backend_dev.md 已协调更新。下一步：Phase 1 旅游顾问端到端。2026-07-10 | [设计](system/image-asset-pipeline-design.md) | [计划](plans/plan-image-asset-pipeline.md) |
