...
| 37 | 图片资产全链路承载能力 | 🔧 部分完成（Phase 0+1+2 已完成，Phase 3 按需启动） | **系统级横切能力**：定义图片资产（Image Asset）从来源/注册/寻址/嵌入/渲染的统一规范。触发场景：旅游顾问行程 Word 缺景点图片。四层架构：① 资源管理（ImageRef + ImageRegistry，复用 cp 的 file_id 体系）② Agent 回复承载（新增 images SSE 事件 + ChatMessage.images + 前端 ImageGallery + 渠道图片消息）③ 知识库图片资产（documents/chunks.metadata JSON 扩展，attraction_search 返回 cover_image）④ 文档生成嵌图（通用 image_inliner 支持 file_id:/kb:// 自定义 scheme，Word/PDF/PPT 共用）。设计原则：复用 cp 基建、来源无关、统一契约 ImageRef。Phase 1 直击旅游顾问，Phase 2 解锁 Agent 回复显图（所有子智能体受益）。与 file_usage.md / cache_usage.md / backend_dev.md / frontend_dev.md 需协调增补。**Phase 0 已完成**（2026-07-13）：ImageRef + ImageRegistry + 15 单测全绿，复用 cp Redis 协议。**Phase 1 已完成**（2026-07-13）：旅游顾问端到端（zip 导入 / attraction_search cover_image / image_inliner / md_to_word 集成 / SUBAGENT.md 图集规范 / 前端景点管理图片上传），62 单测全绿，CodeReview 修复 zip slip + 路径遍历。**Phase 2 已完成**（2026-07-14）：Agent 回复显图能力（make_image_event + UnifiedResponse.images + Agent 主循环推送 + ImageGallery 画廊组件 + MessageItem placement 分区渲染 + feishu/dingtalk 拆分发送占位符 + wecom_kf 整段转长图[同事 07fd592]），133 单测全绿，CodeReview 修复 P0 渠道端收不到图（collected_images 孤儿变量）。**Phase 3 已剥离为独立计划**（[plan-image-asset-pipeline-phase3.md](plans/plan-image-asset-pipeline-phase3.md)）：8 个候选方向（wecom 渠道图片 / wecom_kf 语义对称 / PPT/PDF inliner / 文档解析器内嵌图 / OCR+VLM / LLM 占位符 / kb:// scheme / HTML img），按实际使用反馈渐进投入。遗留：wecom_kf 不读 message.get_images()（Phase 3 1.2 处理）。2026-07-10 | [设计](system/image-asset-pipeline-design.md) · [主计划](plans/plan-image-asset-pipeline.md) · [Phase 3 计划](plans/plan-image-asset-pipeline-phase3.md) |
