...
| 31 | 文件生成类工具入参语义拆分与路由健壮性 | 🔧 部分完成 | Word/PDF/Excel/PPT 当前普遍用 `context` 同时承载“用户目的 + 待处理正文 + 隐含参数”，导致内部 LLM 路由不稳定、转换正文被工具指令污染。目标：新增 `instruction/content/content_type/output_name` 结构化入参，`context` 仅作兼容；规则路由覆盖高确定性场景，内部 LLM 只处理复杂参数抽取。已完成 Word 短期兜底 + Phase 1 正式入参扩展；PDF Phase 2 已完成：`md_to_pdf/html_to_pdf` 优先消费清洗后的 `content`，旧 `context` 自动剥离 Markdown/HTML 指令前缀，Markdown/HTML/DOCX 转 PDF 支持确定性路由；Excel Phase 3 已完成：`export` 数据来源改为 `params.data > content > context提取正文`，Markdown/CSV/JSON 正文提取独立封装，无数据继续返回 `needs_data=True`，CSV/JSON 附件转换仍走 convert；PPT Phase 1 已完成并通过独立 review：新增结构化入参、旧 `context` 拆分、五模式确定性路由和标题优先级，相关单测 28 passed。待补 Agent/旅游顾问提示更新。2026-06-30 | [设计](tools/tool-input-contract-redesign.md) | [开发计划](tools/tool-input-contract-redesign-dev-plan.md) |
...
| 34 | PPT 工具增强与 HTML 转 PPTX 导出 | 🔧 部分完成 | 整合现有 PPT 工具设计、深度调研、HTML 转 PPTX 调研和文件工具入参契约 Phase 4。目标：补齐 `instruction/content/content_type/output_name/export_mode` 结构化入参；直接引入 Node.js + PptxGenJS 作为新增 PPTX 主渲染器；自研与 PptxGenJS 高度对齐的 `SlideDeckSpec` 中间协议，并借鉴 Codex Presentations 插件的 layout registry、模板跟随、frame map、渲染 QA 和对象检查机制；支持 HTML 页面 PPT 导出为高保真截图版 PPTX，以及尽量保真的可编辑 PPTX；不纳入 Google Slides。Phase 0-4 已完成：已落地结构化入参、Python schema/planner 转换、Node/PptxGenJS CLI、默认 Node 渲染及回退、11 类 layout registry；新增 Playwright HTML 字符串/文件截图导出、`section.slide`/横向 deck 分页、PNG/JPEG、RasterLayerNode 高保真 PPTX 和截图/PPTX QA；`high_fidelity` 已接入，`both` 在 Phase 5 前明确 warning 且不伪造 editable。Phase 5 可编辑 HTML 导出仍待开发。2026-06-30 | [设计](tools/ppt/ppt_tool_enhancement_design.md) | [开发计划](tools/ppt/ppt_tool_enhancement_dev_plan.md) |
