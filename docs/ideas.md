...
|---|------|------|------|---------|---------|
| 20260917-1201 | 生产 243 仿真环境（staging） | 🔧 部分完成 | 代码层完成：SIMULATION_MODE 门控（SMTP/通知 dry-run、附件删除跳过、启动横幅）+ 同步脚本 + sim compose + nginx conf；待 243 服务器侧执行（建库建账号、目录初始化、nginx 上线）与 P1 验收。 | [设计](system/simulation-env-design.md) | — |
