...
| 20260918-1930 | 生产主日志 WARNING 审计（09-16~09-18） | 🔧 部分完成 | 生产 3 天主日志 119 条 WARNING 聚合为 8 类；#1 deepseek-flash（93 条）部署时差噪音已解决；#4 sanitizer 降级 INFO + 补 source（8 入口）完成待部署；#3 连续 user 丢弃预览加长 500 字符（含完整 ASR 文本）完成待部署；#6 Redis DNS 降级三层防御（compose 健康依赖 + 启动重试 3 次 + 恢复清残留）完成待部署；agent_update.sh 发版不再连带重启 redis（up 限定 api/background，生产根目录副本已同步）；余下：wechat_mp GET 验签失败（8 条）等。 | — | [审计报告](ops/log-warning-audit-20260918.md) |
...
| 20260918-2100 | 测试环境主日志 WARNING 审计（09-16~09-18） | 🔧 部分完成 | 测试机 254 容器 aid-agent-api2 三天 4840 条 WARNING 聚合 15 类；重点：#1 upsert_db_config 显示名重复刷屏 4627 条（95%，pre-sales vs aidefine-sales-assistant 重名 + 告警无去重）、#9 deepseek-flash 未配单价 credit_cost=0（计费缺口）、#7 wecom_kf ASR HTTP 400（与生产同日疑同因）、#2 RPA 租约过期日递增 24→47→63。余 11 类低频观察/正常业务日志。待逐一处理。 | — | [审计报告](ops/log-warning-audit-20260918-testenv.md) |
