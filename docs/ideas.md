...
| # | 功能 | 状态 | 说明 | 设计文档 | 开发计划 |
|---|------|------|------|---------|---------|
| 20 | 浏览器混合执行、可视化与人工接管 | 🔧 部分完成 | 2026-07-17 Phase 0～1 已完成；Phase 2 已实现、待真实服务门禁：生产主链路切换为 RunManager + 统一 Executor + 每 run 独占 Playwright worker；PageOps 不再持有 Page/BrowserSession；IPC 固定 4 字节大端长度帧与 1 MB 上限，命令具备 deadline、单调 seq、command_id 幂等；新增租户隔离状态机、幂等 finalize、owner lease/reaper、跨 worker cancel 与 Redis 不可用单请求 server 降级；新增 `bs_browser_runs`/`bs_browser_assistance_requests` 双脚本 DDL 和 tenant-scoped DB service。P1 已补写帧前 owner epoch fencing，租约过期/reaper 抢占/同 owner ABA/取消/状态丢失均不写 IPC；Windows worker 已用 `KILL_ON_JOB_CLOSE` Job Object 托管子树且 attach 失败 fail-closed。mock/本机 Playwright 门禁已覆盖；真实 Redis/PostgreSQL 尚未验收，Phase 2 暂不标完成。Phase 3 尚未开始。 | [设计](tools/browser/browser_visualization_design.md) | [开发计划](tools/browser/browser_execution_dev_plan.md) |
