...
| 47 | 工作成果记录 | 📋 待开发 | 子智能体在产生重要工作成果（生成文件、完成业务操作、给出决策建议）时主动登记到 `work_outcomes` 表，租户前台新增"工作成果"菜单。**与工作日报互补**：日报是过程统计（对话数/积分/节省时间），工作成果是产出沉淀（具体生成了什么文件、做了什么操作），续费复盘时两者结合才有完整画面。核心难点"普适性定义工作成果"采用**三层混合机制**：层1 SUBAGENT.md `work_outcomes` 字段显式声明（精度）+ 层2 `subagent_base.md` 通用约束 + LLM 自主判断（兜底覆盖）+ 层3 Agent 主循环监测 `cp` 调用自动捕获文件交付（保险，防 LLM 漏记）。`source` 字段区分 `agent_active`/`auto_captured`/`manual`，`auto_capture_rate` 作为运维指标反向优化提示词。Phase 1 MVP（表+工具+提示词+自动捕获+列表页，3-4 人天）+ Phase 2 声明层+统计（2-3 人天）+ Phase 3 增强（导出/反馈/联动，可选）。2026-07-29 | [设计](system/work-outcome-record-design.md) | - |
