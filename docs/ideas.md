...
| 47 | AI 批量视频内容生产系统 | 📋 待开发 | 以「AI 批量出片」为核心的 C 端内容生产引擎，面向电商运营人员（美妆工具/假睫毛等品类），日处理 5-10 视频、10-20 SKU。定位"重生产、分发放二期"。**v0.2 重大深化（2026-07-29）**：基于深度调研，把产品从"调API"升级为"场景化预设+精准prompt+用户微调"的可用产品。核心新增：①**视频生成工艺链**（10环节流水线：抠图→参考图注入→prompt补全→运镜→I2V→拼接→超分补帧→转场→字幕配音→合规标识，绝非单次API调用，是产品真正壁垒）；②**场景预设库+提示词引擎**（7类电商场景预设封装prompt模板+运镜LoRA+ComfyUI工作流JSON，提示词引擎把用户模糊输入自动扩展为结构化精准prompt，对标可灵8层框架/Seedance公式）；③**主体一致性技术栈**（IP-Adapter+产品LoRA+多图参考+ControlNet，解决产品变形这一电商第一痛点）。**关键修正**：主力时长30-60s→15-30s（完播率数据）；新增AI内容显式标识为P0硬需求（2025.9.1法规，不标识即限流）；批量模式引入反同质化差异化（应对平台"同构图换皮连发"红线）；支持实拍+AI混合素材（美妆品类纯AI是减分项）。**自建选型**：Wan2.2-I2V-A14B（Apache2.0开源SOTA，有Fun-Control运镜控制）。**MVP（P0）8-10周**：工艺链编排+通义万相API+场景预设+提示词引擎+反同质化批量+合规标识+审核台+人工下载。**Phase 1**：自建Wan2.2+ComfyUI工作流引擎+CogVideoX故障转移+抖音/视频号真实发布。**Phase 2**：B端国际站海报+数据回流。诚实定位：辅助投放素材生产+快速试错（纯AI视频规模化带货GMV闭环行业尚未跑通）。复用file_id/MediaRef、background_runner、social_*发布链路；新建product_*/creative_*(scene_presets)/gen_*(含pipeline_steps)、src/generation/层。PRD v0.2完成，待评审。2026-07-29 | [调研](research/ai-video-production-research.md) / [PRD](system/content-production/ai-video-production-prd.md) | — |
