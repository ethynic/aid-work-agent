...
| 34 | PPT 工具增强与 HTML 转 PPTX 导出 | 🔧 部分完成 | Phase 0-7 已完成：已落地结构化入参、SlideDeckSpec、Node/PptxGenJS 渲染及回退、HTML 高保真/可编辑双模式和可审计模板跟随；新增统一 `PPTQualityValidator`，检查文件可打开性、页数、空白页、对象越界、图片/raster 可读性，优先使用 LibreOffice/Poppler 生成 preview，缺失时安全降级；Node/Python 路径统一输出脱敏 `.layout.json`、`.qa-report.json`，工具稳定返回 `qa_summary`、`editable_ratio` 和 warning，`PPT_QA_STRICT` 可在严重失败时阻止交付。真实 Playwright、模板、损坏 PPTX、strict/非 strict、双输出及重开测试已通过。Phase 8 Agent 提示与文档收尾仍待开发。2026-07-01 | [设计](tools/ppt/ppt_tool_enhancement_design.md) | [开发计划](tools/ppt/ppt_tool_enhancement_dev_plan.md) |
