...
| 37 | 图片资产全链路承载能力 | 📋 待开发 | **系统级横切能力**：定义图片资产（Image Asset）从来源/注册/寻址/嵌入/渲染的统一规范。触发场景：旅游顾问行程 Word 缺景点图片。四层架构：① 资源管理（ImageRef + ImageRegistry，复用 cp 的 file_id 体系）② Agent 回复承载（新增 images SSE 事件 + ChatMessage.images + 前端 ImageGallery + 渠道图片消息）③ 知识库图片资产（documents/chunks.metadata JSON 扩展，attraction_search 返回 cover_image）④ 文档生成嵌图（通用 image_inliner 支持 file_id:/kb:// 自定义 scheme，Word/PDF/PPT 共用）。设计原则：复用 cp 基建、来源无关、统一契约 ImageRef。Phase 1 直击旅游顾问，Phase 2 解锁 Agent 回复显图（所有子智能体受益）。与 file_usage.md / cache_usage.md / backend_dev.md / frontend_dev.md 需协调增补。2026-07-10 | [设计](system/image-asset-pipeline-design.md) | — |
...
> 2026-07-10 补充：RPA 出站协议升级至 v1.1，`ActionEnvelope.reply_context` 明确携带发送人、原始入站文本、Agent 回复和桌面端会话搜索名；客户端使用去除末尾 `@微信` 的搜索名定位窗口，缺少可靠搜索名时拒绝盲发。服务端补齐已鉴权 config→client→租户命名空间 account 映射、在线/离线 `type=actions` 信封、outbox 上下文持久化和真实 `send_ok`，前端支持维护会话搜索名。WS 配置标识回归已修复：新客户端统一使用 `chan_*` 业务 ID，服务端兼容历史数字 ID，并严格按 tenant/channel/client 归属解析，未归属或他人配置不再通过 `/config` 泄漏。状态仍为 🔧 部分完成，待 agent2 真机验证完整下发与执行回执。
