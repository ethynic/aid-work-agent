...
| 34 | PPT 工具增强与 HTML 转 PPTX 导出 | 🔧 部分完成 | Phase 0-5 已完成：已落地结构化入参、Python schema/planner 转换、Node/PptxGenJS CLI、默认 Node 渲染及回退、11 类 layout registry；支持 Playwright HTML 字符串/文件高保真导出，以及基于 DOM rect/computed style/selector/z-index 的尽量可编辑导出；文本、基础形状、图片/SVG、表格原生重建，canvas、复杂 SVG/filter/backdrop-filter/mix-blend-mode 局部栅格回退；支持可编辑率统计和 `high_fidelity/editable/both` 完整行为。真实 Playwright、双 PPTX 输出、临时目录清理及 PPTX 重开测试已通过。Phase 6 模板跟随增强及后续质量验证仍待开发。2026-06-30 | [设计](tools/ppt/ppt_tool_enhancement_design.md) | [开发计划](tools/ppt/ppt_tool_enhancement_dev_plan.md) |
