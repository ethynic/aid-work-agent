...
- [x] CLI `goto recommend|chat` 子命令（`PageNavigator`）：DOMSnapshot 定位左侧菜单（doc0 + x<200 侧栏区，排除顶部同文案诱饵）→ Win32 点击 → target URL 校验跳转结果（推荐牛人=/web/chat/recommend，沟通=/web/chat/index，文本判定会被沟通页内嵌推荐 iframe 骗过故用 URL）；已在目标页幂等跳过。真机验证通过（2026-08-05：沟通↔推荐牛人双向跳转 + 幂等跳过）。用途：页面不在推荐牛人时先跳回再 filter/greet；打完招呼跳沟通看回复。
