...
| `lgx` | `w-[85vw] h-[80vh]` | 弹框中再开弹框的内层表单（如渠道配置弹框内的客服账号编辑） | 一般一行 3~4 个字段 |
| `xl` | `w-[92vw] h-[95vh]` | 内容较多的详情页 | 一般一行 4 个字段，如果是大文本字段可以一行 1 个 |

弹框总高默认上限 `max-h-[90vh]`，内容区（body）通过 `flex-1 min-h-0` 填满剩余空间并带纵向滚动条（`scrollable=true`）。任何分辨率下弹框总高都不会超过视口高度，header/footer 始终可见。
