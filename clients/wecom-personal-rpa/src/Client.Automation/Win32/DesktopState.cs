...
internal sealed class DesktopState
{
    ...
    {
        // 屏幕保护运行检测：SPI_GETSCREENSAVERRUNNING 的 pvParam 是 BOOL*。
        bool isSsRunning = false;
        if (SystemParametersInfoBoolExtension.SystemParametersInfoGetScreensaver(
                NativeMethods.SPI_GETSCREENSAVERRUNNING, 0, ref isSsRunning, 0))
        {
            if (isSsRunning)
            {
                return true;
    ...
    public (int X, int Y, int Width, int Height) GetWorkArea()
    {
        ...
        SystemParametersInfoRectExtension.SystemParametersInfoGetWorkArea(
            NativeMethods.SPI_GETWORKAREA, 0, ref rc, 0);
        return (rc.Left, rc.Top, rc.Width, rc.Height);
    }
}

internal static class SystemParametersInfoBoolExtension
{
    [DllImport("user32.dll", SetLastError = true, CharSet = CharSet.Auto)]
    [return: MarshalAs(UnmanagedType.Bool)]
    internal static extern bool SystemParametersInfoGetScreensaver(
        uint uiAction, uint uiParam, ref bool pvParam, uint fWinIni);
}

internal static class SystemParametersInfoRectExtension
{
    [DllImport("user32.dll", SetLastError = true, CharSet = CharSet.Auto)]
    [return: MarshalAs(UnmanagedType.Bool)]
    internal static extern bool SystemParametersInfoGetWorkArea(
        uint uiAction, uint uiParam, ref NativeMethods.RECT pvParam, uint fWinIni);
}
