...
test('mcp-conformance 套件全项通过', { timeout: 120000 }, async () => {
  ...
  const report = await runConformance({
    ...
    expectTools: ['weixin_probe', 'weixin_chat_search', 'weixin_message_send', 'weixin_history_read', 'weixin_unread_list'],
...
test('mcp-conformance 套件 CLI 模式可独立运行', { timeout: 60000 }, async () => {
  ...
  execFileSync(
    ...
    [
      ...
      '--expect-tools',
      'weixin_probe,weixin_chat_search,weixin_message_send,weixin_history_read,weixin_unread_list',
      '--call',
      'weixin_probe:{}',
      '--invalid',
