/**
 * CLI 子命令 doctor：只读环境检查（上位规范 §4 强制命令面 / 设计 §4.1）。
 *
 * 用法：
 ...
 * 信息项（不影响退出码）：Weixin.exe 进程存在性。
 *
 * 严格只读：不激活窗口、不发送按键、不改剪贴板、不打开搜一搜。
 */
import {
  artifactDir,
...
export async function runDoctorChecks(opts: DoctorCommandOptions = {}): Promise<DoctorReport> {
  ...
  checks.push({
    name: 'Weixin.exe 进程（信息项）',
    ok: true,
    severity: 'info',
    detail: env.weixin_running ? `运行中（${env.weixin_processes} 个进程）` : '未检测到进程（执行业务操作前请启动并登录微信）',
  })
