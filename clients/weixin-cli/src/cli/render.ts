 ...
 * 退出码（成功 0 / 参数 2 / 其余 1）。
 *
 * exclusive=true（驱动微信 UI 的命令必须开启）：执行前占用跨进程命名互斥
 * （与 MCP server 同一 scope，设计 §6.1：用户操作与 Provider 调用不得并行
 * 控制微信）；被占用打印 BUSY 并以退出码 1 终止。
 */
...
export interface RunCliOperationOptions {
  /** 驱动微信 UI 的命令必须跨进程互斥（与 MCP server 同一 NamedMutex scope） */
  exclusive?: boolean
...
export async function runCliOperation<Args>(
  ...
  try {
    if (opts.exclusive) {
      ...
      if (!acquired) {
        console.error('❌ 另一个 aid-weixin 进程正在操作微信（跨进程互斥），请等其完成后重试（code=BUSY，可重试）')
        return 1
      }
    }
    const ctx: OpContext = {
