/**
 * 跨进程互斥（设计 §6.1：一个进程内单飞，加跨进程命名对象）。
 *
 * 实现：Windows 命名管道 `\\.\pipe\AidWorkAgent.AidWeixin.<scope>`。
...
const PIPE_PREFIX = '\\\\.\\pipe\\AidWorkAgent.AidWeixin.'
const SCOPE_PATTERN = /^[A-Za-z0-9_-]+$/

/** 互斥 scope：默认 session（保护交互会话/前台/剪贴板这一共享资源）；测试可用 env 覆盖做并行隔离 */
export function resolveMutexScope(): string {
  return process.env.AID_WEIXIN_MUTEX_SCOPE || 'session'
}
...
export function mutexPath(scope: string, platform: NodeJS.Platform = process.platform): string {
  ...
  return join(tmpdir(), `AidWorkAgent.AidWeixin.${scope}.sock`)
}
