...
操作命令：
  ...
  send-to <姓名> --message <消息> [--dry-run]   打开「姓名」会话（统一切换：already/搜索/列表兜底+身份校验）→ 输入并发送（默认真发送；--dry-run 只输入不发送；期间勿动鼠标）
  ...
  resume-detail --name <姓名> [--save-image <path>]   读取当前打开的候选人简历详情（canvas 截图拼接 OCR；--name 候选人姓名必传，缺省报错，且会与 OCR 文本交叉校验；前提先点开候选人详情；只读，但滚动借用真实鼠标，期间勿动鼠标）
