...
4. **双通道点击**：浏览类（卡片/Escape/滚动）走 CDP `Input.dispatchMouseEvent`；**写动作按钮 + 筛选类控件走 Win32 真实鼠标**（`WinMouseClicker`，BOSS 反作弊 SDK 选择性拦截 CDP 合成点击，§16 决策8）。
...
| 层 | 职责 | 参照 |
|---|---|---|
| **Executor** `src/main/boss/` | 纯逻辑，依赖注入 `Deps`（snapshot/click/scroll/pressEscape/typeChar/signal），可单测 | `GreetExecutor.ts`、`ChatRejectExecutor.ts`、`ResumeConsentExecutor.ts`、`ChatSearchExecutor.ts`、`ChatSendExecutor.ts` |
