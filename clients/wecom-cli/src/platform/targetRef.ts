 ...
 * payload：{v:1, name, type, subtitle, exp}（exp = 签发 + 5 分钟，unix 秒）。
 * - name：结果项名称（联系人/群名）；
...
export type CreateTargetRefFn = (name: string, type: string, subtitle?: string) => string
...
/** 签发 target_ref（type：contact/group/other；subtitle 缺省空串） */
export function createTargetRef(name: string, type: string, subtitle = '', opts: TargetRefOptions = {}): string {
...
export function verifyTargetRef(ref: string, opts: TargetRefOptions = {}): TargetRefIdentity {
  ...
  return { name: payload.name, type: payload.type, subtitle: payload.subtitle }
}
